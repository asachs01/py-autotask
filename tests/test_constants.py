...
class TestEnumIntegrity:
    ...
    def test_no_duplicate_values_in_enums(self):
        ...
        enums_to_test = [
            ...
            ContractStatus, ContractType, ContractBillingType,
            ResourceStatus, ResourceType,
            AccountStatus, AccountType,
            ExpenseReportStatus,
            TimeEntryType, TimeEntryStatus,
    ...
    def test_enum_values_are_positive_integers(self):
        ...
        for enum_class in enums_to_test:
            for item in enum_class:
                assert isinstance(item.value, int), f"{enum_class.__name__}.{item.name} value is not int"
                assert item.value > 0, f"{enum_class.__name__}.{item.name} value is not positive"
            ...
            if hasattr(constants_class, 'STATUS_FILTERS'):
                ...
                for filter_name, status_list in status_filters.items():
                    for status_value in status_list:
                        assert status_value in valid_values, \
