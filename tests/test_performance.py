...
class TestPaginationPerformance:
    ...
    def test_pagination_memory_efficiency(self, mock_client):
        ...
        print(f"Processed {total_items} items, memory increase: {memory_increase:.2f}MB")
    ...
    def test_pagination_speed(self, mock_client):
        ...
        print(f"Processed {total_items} items in {total_time:.2f}s ({items_per_second:.1f} items/s)")
...
class TestBatchOperationPerformance:
    ...
    def test_batch_create_performance(self, mock_client):
        ...
        # Test data
        large_dataset = [{'title': f'Item {i}', 'description': f'Description {i}'} 
                        for i in range(1000)]
        ...
        print(f"Created {len(large_dataset)} items in {total_time:.2f}s ({items_per_second:.1f} items/s)")
    ...
    def test_batch_operation_memory_usage(self, mock_client):
        ...
        print(f"Processed {len(large_dataset)} items, memory increase: {memory_increase:.2f}MB")
...
class TestQueryPerformance:
    ...
    def test_filter_processing_performance(self, mock_client):
        ...
        assert avg_time_per_query < 0.01, f"Average query time: {avg_time_per_query:.4f}s"
        
        print(f"Processed 100 filtered queries in {total_time:.3f}s (avg: {avg_time_per_query:.4f}s)")
    ...
    def test_large_result_set_handling(self, mock_client):
        ...
        large_items = [{'id': i, 'title': f'Ticket {i}', 'description': 'x' * 500} 
        ...
        print(f"Processed {len(result.items)} items in {processing_time:.2f}s, memory: +{memory_increase:.2f}MB")
...
class TestConnectionPerformance:
    ...
    def test_connection_pool_efficiency(self):
        ...
        with patch('requests.Session') as mock_session_class:
            ...
            mock_session.get.return_value = Mock(status_code=200, json=lambda: {'items': []})
            mock_session_class.return_value = mock_session
            ...
            assert requests_per_second > 1000, f"Only {requests_per_second:.1f} requests/second"
            
            print(f"Made 100 requests in {total_time:.4f}s ({requests_per_second:.1f} req/s)")
...
class TestMemoryPerformance:
    ...
    def test_entity_creation_memory_efficiency(self):
        ...
        assert creation_memory < 50, f"Created 1000 entities using {creation_memory:.2f}MB"
        assert cleanup_memory < creation_memory / 2, f"Memory not properly cleaned up"
        
        print(f"1000 entities: +{creation_memory:.2f}MB, after cleanup: +{cleanup_memory:.2f}MB")
...
if __name__ == "__main__":
    ...
    passed_tests = sum(1 for suite_results in results.values() 
                      for result in suite_results.values() if result is not None)
