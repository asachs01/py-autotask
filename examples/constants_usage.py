...
def demonstrate_utility_functions():
    ...
    try:
        ...
        print(
            f"  Task 'completed' filter: {[status.value for status in task_completed]}"
        )
...
def demonstrate_backwards_compatibility():
    ...
    print(
        f"  TASK_STATUS_COMPLETE == TaskStatus.COMPLETE: {TASK_STATUS_COMPLETE == TaskStatus.COMPLETE}"
    )
    print(
        f"  TASK_PRIORITY_CRITICAL == TaskPriority.CRITICAL: {TASK_PRIORITY_CRITICAL == TaskPriority.CRITICAL}"
    )
    print()
