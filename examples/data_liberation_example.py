...
async def demo_basic_operations(client):
    ...
    try:
        ...
        console.print(f"✅ Found {len(recent_tickets.items)} active tickets (cached for performance)")
...
async def demo_concurrent_operations(client):
    ...
    try:
        ...
        companies_task = client.companies.query_async({"filters": [{"field": "isActive", "op": "eq", "value": "true"}]})
        ...
        console.print(f"✅ Concurrent operations completed in {duration.total_seconds():.2f}s")
...
async def demo_pandas_integration(client):
    ...
    try:
        ...
        df_tickets = await pandas_integration.to_dataframe(
            entity_type="tickets",
            filters=[{"field": "createDate", "op": "gte", "value": "2024-01-01T00:00:00Z"}],
            max_records=100,
        ...
        if not df_tickets.empty:
            ...
            analytics_table.add_row("Total Tickets", str(analytics.get("total_tickets", "N/A")))
            analytics_table.add_row("Priority Distribution", str(len(analytics.get("priority_distribution", {}))) + " categories")
            analytics_table.add_row("Status Distribution", str(len(analytics.get("status_distribution", {}))) + " statuses")
            
            if "avg_resolution_time_hours" in analytics:
                analytics_table.add_row("Avg Resolution Time", f"{analytics['avg_resolution_time_hours']:.1f} hours")
...
if __name__ == "__main__":
    ...
    required_env_vars = ["AUTOTASK_USERNAME", "AUTOTASK_INTEGRATION_CODE", "AUTOTASK_SECRET"]
