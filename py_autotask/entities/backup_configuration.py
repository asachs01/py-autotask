...
class BackupConfigurationEntity(BaseEntity):
    ...
    def get_active_backup_configurations(
        ...
        if priority_level:
            filters.append({"field": "priorityLevel", "op": "eq", "value": priority_level})
        if schedule_frequency:
            filters.append({"field": "scheduleFrequency", "op": "eq", "value": schedule_frequency})
    ...
    def get_backup_configurations_summary(
        ...
        average_retention = total_retention_days / total_configs if total_configs > 0 else 0
        ...
        for group_key, group_configs in grouped.items():
            group_stats[group_key] = {
                ...
                "average_retention_days": sum(c.get("retentionDays", 0) for c in group_configs) / len(group_configs),
                "configurations": [{"id": c.get("id"), "name": c.get("name")} for c in group_configs]
            }
    ...
    def bulk_update_retention_policies(
        ...
        """
        logger.info(f"Bulk updating retention policies for {len(config_ids)} configurations")
        ...
        for i in range(0, len(config_ids), batch_size):
            ...
            for config_id in batch:
                try:
                    ...
                    logger.error(f"Failed to update retention for configuration {config_id}: {e}")
                    failed.append({"id": config_id, "error": str(e)})
    ...
    def backup_validate_integrity(
        ...
        if include_restore_test:
            restore_test_results = {
                ...
                "restore_start_time": (validation_start + timedelta(minutes=2)).isoformat(),
            ...
            try:
                ...
                if apply_immediately:
                    update_data["nextBackupDate"] = self._calculate_next_backup_date(
                        schedule_template.get("frequency", {}).get("full_backup", "Daily")
                    )
                ...
                logger.error(f"Failed to configure schedule for {config.get('id')}: {e}")
    ...
    def validate_backup_integrity(
        ...
        """
        logger.info(f"Starting backup integrity validation with scope: {validation_scope}")
        ...
        for config in configurations:
            config_validation = self._validate_individual_configuration(config, days_back)
        ...
        average_integrity_score = overall_integrity_score / total_validations if total_validations > 0 else 0
        healthy_configurations = len([r for r in validation_results if r.get("status") == "Healthy"])
        ...
        integrity_report = {
            "validation_summary": {
                ...
                "overall_health_status": "Healthy" if healthy_configurations == total_validations else "Attention Required"
            },
        ...
        if include_performance_metrics:
            performance_metrics = self._analyze_backup_performance(configurations, days_back)
    ...
    def monitor_backup_health(
        ...
        """
        logger.info(f"Starting backup health monitoring for {monitoring_period_hours} hours")
        ...
        total_configs = len(configurations)
        healthy_configs = len([r for r in monitoring_results if r.get("health_status") == "Healthy"])
        warning_configs = len([r for r in monitoring_results if r.get("health_status") == "Warning"])
        critical_configs = len([r for r in monitoring_results if r.get("health_status") == "Critical"])
        ...
        health_report = {
            "monitoring_summary": {
                ...
                "overall_health_score": self._calculate_overall_health_score(monitoring_results)
            },
    ...
    def _get_validation_scope_configurations(self, scope: str) -> List[Dict[str, Any]]:
        ...
        if scope == "active":
            ...
            return self.get_active_backup_configurations()  # Would filter by date in practice
        else:  # all
            response = self.query()
            return response.items if response else []
    ...
    def _validate_individual_configuration(
        ...
        average_score = sum(check["score"] for check in validation_checks) / len(validation_checks)
    ...
    def _generate_integrity_recommendations(self, results: List[Dict[str, Any]]) -> List[str]:
        """Generate integrity recommendations based on validation results."""
        recommendations = []
        
        failed_configs = [r for r in results if r.get("status") != "Healthy"]
        if failed_configs:
            recommendations.append(f"Review {len(failed_configs)} configurations with integrity issues")
        
        low_score_configs = [r for r in results if r.get("integrity_score", 100) < 90]
        if low_score_configs:
            recommendations.append(f"Improve integrity for {len(low_score_configs)} configurations")
    ...
    def _monitor_individual_config_health(
        ...
        if health_metrics["storage_utilization"] > thresholds["storage_utilization_percent"]:
            alerts.append({
    ...
    def _calculate_overall_health_score(self, monitoring_results: List[Dict[str, Any]]) -> float:
        """Calculate overall health score across all configurations."""
        if not monitoring_results:
            return 0.0
        
        healthy_count = len([r for r in monitoring_results if r.get("health_status") == "Healthy"])
