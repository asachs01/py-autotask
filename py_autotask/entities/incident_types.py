...
class IncidentTypesEntity(BaseEntity):
    ...
    def clone_incident_type(
        ...
        clone_data = {k: v for k, v in source_incident_type.items() 
    ...
    def get_incident_types_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_severity": {severity: len(types) for severity, types in severity_groups.items()},
            "by_priority": {priority: len(types) for priority, types in priority_groups.items()},
    ...
    def bulk_update_escalation_times(
        ...
        for update in escalation_updates:
            ...
            try:
                result = self.update_escalation_time(incident_type_id, escalation_minutes)
                results.append({"id": incident_type_id, "success": True, "result": result})
            except Exception as e:
                results.append({"id": incident_type_id, "success": False, "error": str(e)})
