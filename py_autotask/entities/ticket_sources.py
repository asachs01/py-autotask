...
class TicketSourcesEntity(BaseEntity):
    ...
    def get_active_sources(self, include_inactive: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def get_source_by_integration_code(self, integration_code: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_source_usage_stats(self, source_id: int, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
    ...
    def get_source_comparison(self, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        for source in sources:
            ...
            comparison['by_source'][source_id] = {
                'name': name,
                'stats': stats
            }
    ...
    def set_source_order(self, source_orders: List[Dict[str, Any]]) -> List[UpdateResponse]:
        """
    ...
    def create_integration_source(self, integration_data: Dict[str, Any]) -> CreateResponse:
        """
    ...
    def update_automation_rules(self, source_id: int, automation_rules: Dict[str, Any]) -> UpdateResponse:
        """
        ...
        import json
        update_data = {
            'automationRules': json.dumps(automation_rules)
        }
    ...
    def bulk_update_sources(self, source_updates: List[Dict[str, Any]]) -> List[UpdateResponse]:
        """
        ...
        for update_info in source_updates:
            ...
            if source_id and update_data:
                try:
                    ...
                    self.client.logger.error(f"Failed to update source {source_id}: {e}")
                    results.append({'error': str(e), 'source_id': source_id})
        
        return results
    
    def get_ticket_source_trends(self, date_range: Optional[tuple] = None, granularity: str = 'month') -> Dict[str, Any]:
        """
        ...
        trends = {
            ...
            'date_range': {
                'start': date_range[0].isoformat() if date_range and hasattr(date_range[0], 'isoformat') else date_range[0] if date_range else None,
                'end': date_range[1].isoformat() if date_range and hasattr(date_range[1], 'isoformat') else date_range[1] if date_range else None
            },
            'by_source': {},
            'timeline': {}
        }
            ...
            for ticket in tickets:
                ...
                if create_date:
                    try:
                        ...
                        source_timeline[period_key] = source_timeline.get(period_key, 0) + 1
                        
                        # Add to overall timeline
                        if period_key not in trends['timeline']:
                            trends['timeline'][period_key] = {}
                        trends['timeline'][period_key][source_name] = source_timeline[period_key]
    ...
    def validate_source_data(self, source_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if integration_code:
            ...
            if existing_source and existing_source.get('id') != source_data.get('id'):
                warnings.append(f"Integration code '{integration_code}' is already in use")
