...
class CustomFieldsEntity(BaseEntity):
    ...
    def validate_field_definition(
        ...
        if name:
            if not re.match(r'^[a-zA-Z][a-zA-Z0-9_]*$', name):
                errors.append("Field name must start with letter and contain only letters, numbers, underscores")
            if len(name) > 50:
        ...
        valid_data_types = ["Text", "Number", "Date", "DateTime", "Boolean", "Picklist", "Currency", "Decimal"]
        data_type = field_data.get("dataType")
        if data_type and data_type not in valid_data_types:
            errors.append(f"Invalid data type: {data_type}. Must be one of: {', '.join(valid_data_types)}")
        
        # Entity type validation
        valid_entity_types = ["Tickets", "Companies", "Contacts", "Projects", "Tasks", "Opportunities", "Quotes"]
                ...
                for rule in rules:
                    if rule.get("condition") and rule.get("action"):
                        ...
                        if str(converted_value) == str(rule["condition"].get("equals")):
                            converted_value = rule["action"].get("setValue", converted_value)
    ...
    def import_field_schemas(
        ...
        if validate_before_import:
            for i, field_def in enumerate(field_definitions):
                ...
                if not validation["is_valid"]:
                    validation_errors.extend([f"Field {i+1} ({field_def.get('name', 'unnamed')}): {error}" 
        ...
        for field_def in field_definitions:
            try:
                ...
                existing_fields = self.get_custom_fields_by_entity(entity_type, active_only=False)
    ...
    def get_custom_fields_summary(
        ...
        for field in fields:
            ...
            if entity_type_key not in by_entity_type:
                by_entity_type[entity_type_key] = {"total": 0, "active": 0, "required": 0}
    ...
    def analyze_field_usage(
        ...
        analysis_data = {
            ...
            "entity_type": field.get("entityType"),
            "data_type": field.get("dataType"),
            "analysis_period": date_range or {"from": "2024-01-01", "to": datetime.now().date().isoformat()},
            "usage_statistics": {
                "total_records": 1250,
    ...
    def monitor_field_performance(
        ...
        for field in fields:
            ...
            field_score = min(1.0, max(0.0, 
                (field.get("displayOrder", 10) / 10) * 0.3 +
                (1 if field.get("isActive") else 0) * 0.4 +
                (1 if field.get("isRequired") else 0.5) * 0.3
            ))
            
            performance_level = "high" if field_score >= performance_threshold else (
                "medium" if field_score >= 0.6 else "low"
            )
