...
class HolidaySetsEntity(BaseEntity):
        ...
        if copy_holidays and new_holiday_set_id:
            ...
            for holiday in original_holidays:
                ...
                try:
                    ...
                    self.logger.error(f"Failed to copy holiday {holiday.get('Name')}: {e}")
    ...
    def get_holiday_set_summary(self, holiday_set_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'next_holiday': {
                ...
                'days_until': (datetime.fromisoformat(next_holiday['HolidayDate']).date() - today).days
    ...
    def validate_holiday_set(self, holiday_set_id: int) -> Dict[str, Any]:
            ...
            for holiday in holidays:
                if holiday.get('HolidayDate'):
                    try:
                        ...
                        warnings.append(f"Invalid date format in holiday: {holiday.get('Name')}")
            
            if current_year not in years:
                warnings.append(f'No holidays defined for current year ({current_year})')
        ...
        return {
            ...
            'years_covered': len(set(datetime.fromisoformat(h['HolidayDate']).year 
                                  for h in holidays if h.get('HolidayDate')))
        }
    ...
    def get_holidays_impacting_date_range(
        ...
        return {
            ...
            'impact_percentage': round((weekday_holidays / weekdays * 100) if weekdays > 0 else 0, 2)
        }
        ...
        for i in range(0, len(updates), batch_size):
            ...
            for update in batch:
                try:
                    ...
                    self.logger.error(f"Failed to update holiday set {update.get('id')}: {e}")
                    continue
    ...
    def get_country_distribution(self) -> Dict[str, Any]:
        ...
        for holiday_set in all_sets:
            ...
            if holiday_set.get('IsActive'):
                active_country_counts[country] = active_country_counts.get(country, 0) + 1
        ...
        return {
            ...
            'countries_with_active_sets': len([c for c in active_country_counts.values() if c > 0])
        }