...
class TicketPrioritiesEntity(BaseEntity):
    ...
    def create(self, priority_data: Dict[str, Any]) -> CreateResponse:
        ...
        if priority_level not in [1, 2, 3, 4]:
            raise ValueError("Priority level must be 1 (Critical), 2 (High), 3 (Medium), or 4 (Low)")
    ...
    def get_active_priorities(self, include_inactive: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def get_priority_usage_stats(self, priority_id: int, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        if date_range:
            ...
            filters.extend([
                QueryFilter(field='createDate', op='gte', value=start_date.isoformat() if hasattr(start_date, 'isoformat') else start_date),
                QueryFilter(field='createDate', op='lte', value=end_date.isoformat() if hasattr(end_date, 'isoformat') else end_date)
            ])
        ...
        if stats['closed_tickets'] > 0:
            stats['sla_compliance_rate'] = ((stats['closed_tickets'] - sla_violations) / stats['closed_tickets']) * 100
        
        return stats
    
    def get_priority_comparison(self, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        for priority in priorities:
            ...
            if stats['sla_compliance_rate'] < 100:
                violation_count = stats['closed_tickets'] * (1 - stats['sla_compliance_rate'] / 100)
                total_sla_violations += violation_count
        
        # Calculate overall averages
        if all_resolution_times:
            comparison['summary']['avg_resolution_time'] = sum(all_resolution_times) / len(all_resolution_times)
        
        if total_closed > 0:
            comparison['summary']['overall_sla_compliance'] = ((total_closed - total_sla_violations) / total_closed) * 100
        
        return comparison
    
    def set_priority_order(self, priority_orders: List[Dict[str, Any]]) -> List[UpdateResponse]:
        """
    ...
    def bulk_update_sla_times(self, sla_updates: List[Dict[str, Any]]) -> List[UpdateResponse]:
        """
        ...
        for update_info in sla_updates:
            ...
            if priority_id and sla_data:
                try:
                    ...
                    self.client.logger.error(f"Failed to update priority {priority_id}: {e}")
                    results.append({'error': str(e), 'priority_id': priority_id})
        
        return results
    
    def calculate_sla_metrics(self, priority_id: int, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        if response_times:
            ...
            total_responses = metrics['response_metrics']['within_sla'] + metrics['response_metrics']['breached_sla']
            if total_responses > 0:
                metrics['response_metrics']['compliance_rate'] = (metrics['response_metrics']['within_sla'] / total_responses) * 100
        
        if resolution_times:
            metrics['resolution_metrics']['avg_resolution_time'] = sum(resolution_times) / len(resolution_times)
            total_resolutions = metrics['resolution_metrics']['within_sla'] + metrics['resolution_metrics']['breached_sla']
            if total_resolutions > 0:
                metrics['resolution_metrics']['compliance_rate'] = (metrics['resolution_metrics']['within_sla'] / total_resolutions) * 100
    ...
    def validate_priority_data(self, priority_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if priority_level is not None:
            if priority_level not in [1, 2, 3, 4]:
                errors.append("Priority level must be 1 (Critical), 2 (High), 3 (Medium), or 4 (Low)")
