...
class NotesEntity(BaseEntity):
    ...
    def get_by_creator(
        ...
        """
        filters = [QueryFilter(field='creatorResourceID', op='eq', value=creator_resource_id)]
        
        if date_range:
            start_date, end_date = date_range
            filters.extend([
                QueryFilter(field='noteDateTime', op='gte', value=start_date.isoformat() if hasattr(start_date, 'isoformat') else start_date),
                QueryFilter(field='noteDateTime', op='lte', value=end_date.isoformat() if hasattr(end_date, 'isoformat') else end_date)
            ])
    ...
    def search_notes(
        ...
        filters.append(QueryFilter(field='description', op='contains', value=search_text))
        ...
        for note in notes:
            ...
            if note_date:
                try:
                    ...
                    stats['notes_per_day'][day_key] = stats['notes_per_day'].get(day_key, 0) + 1
                except ValueError:
                    pass
    ...
    def get_entity_note_summary(
        ...
        # Sort notes by date
        sorted_notes = sorted(notes, key=lambda x: x.get('noteDateTime', ''), reverse=True)
    ...
    def validate_note_data(self, note_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if description:
            if len(description) < 1:
                ...
                warnings.append("Note description is very long (over 10,000 characters)")
        
        # Validate note type
        note_type = note_data.get('noteType')
        if note_type is not None:
            if note_type not in [1, 2, 3]:
                errors.append("Note type must be 1 (General), 2 (Important), or 3 (Technical)")
        
        # Validate entity type
        entity_type = note_data.get('entityType', '')
        valid_entity_types = ['Ticket', 'Account', 'Project', 'Contact', 'Contract', 'Task']
