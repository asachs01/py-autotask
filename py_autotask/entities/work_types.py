...
class WorkTypesEntity(BaseEntity):
    ...
    def get_work_type_analytics(
        ...
        """
        if work_type_ids:
            work_type_filter = " or ".join([f"id eq {wt_id}" for wt_id in work_type_ids])
        ...
        for item in analytics:
            if total_hours > 0:
                item["percentage_of_total"] = float(item["total_hours"] / total_hours * 100)
        ...
        return {
            ...
            "summary": {
                ...
                "billable_work_types": len([wt for wt in work_types if wt.get("isBillable")])
            },
            "analytics": analytics
        }
    ...
    def bulk_update_billability(
        self,
        billability_updates: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    def copy_work_type(
        ...
        copy_data = {k: v for k, v in source_work_type.items() 
