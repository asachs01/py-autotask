...
class ProjectTemplatesEntity(BaseEntity):
    ...
    def get_project_template_summary(self, template_id: int) -> Dict[str, Any]:
        ...
        total_estimated_hours = sum(float(task.get('EstimatedHours', 0)) for task in tasks)
        ...
        try:
            ...
            usage_count = len(usage_response.items if hasattr(usage_response, 'items') else usage_response)
        except Exception:
            usage_count = 0
    ...
    def validate_project_template(self, template_id: int) -> Dict[str, Any]:
        ...
        if tasks_without_estimates:
            warnings.append(f'{len(tasks_without_estimates)} tasks without hour estimates')
    ...
    def get_template_usage_statistics(self) -> Dict[str, Any]:
        ...
        most_used = max(usage_stats.items(), key=lambda x: x[1]) if usage_stats else ('None', 0)
        least_used = min(usage_stats.items(), key=lambda x: x[1]) if usage_stats else ('None', 0)
        
        return {
            'total_templates': len(all_templates),
            'active_templates': len([t for t in all_templates if t.get('IsActive')]),
            'total_projects_created': total_usage,
            'average_usage_per_template': round(total_usage / len(all_templates), 2) if all_templates else 0,
            'most_used_template': {'name': most_used[0], 'usage_count': most_used[1]},
            'least_used_template': {'name': least_used[0], 'usage_count': least_used[1]},
