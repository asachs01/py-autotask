...
class TicketStatusesEntity(BaseEntity):
    ...
    def get_active_statuses(self, include_inactive: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def set_status_order(self, status_orders: List[Dict[str, Any]]) -> List[UpdateResponse]:
        """
    ...
    def get_status_usage_stats(self, status_id: int, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
    ...
    def get_status_transitions(self, from_status_id: int, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        if date_range:
            start_date, end_date = date_range
            filters.extend([
                QueryFilter(field='lastActivityDate', op='gte', value=start_date.isoformat() if hasattr(start_date, 'isoformat') else start_date),
                QueryFilter(field='lastActivityDate', op='lte', value=end_date.isoformat() if hasattr(end_date, 'isoformat') else end_date)
            ])
        ...
        for ticket in tickets:
            ...
            current_statuses[current_status] = current_statuses.get(current_status, 0) + 1
        ...
        for update_info in status_updates:
            ...
            if status_id and update_data:
                try:
                    ...
                    self.client.logger.error(f"Failed to update status {status_id}: {e}")
