...
class DataIntegrationsEntity(BaseEntity):
    ...
    def get_active_integrations(
        ...
        if external_system:
            filters.append({'field': 'externalSystem', 'op': 'eq', 'value': external_system})
        if sync_direction:
            filters.append({'field': 'syncDirection', 'op': 'eq', 'value': sync_direction})
    ...
    def configure_integrations(
        ...
        self.logger.info(f"Configuring integration {integration_id} with {len(field_mappings)} field mappings")
    ...
    def sync_data(
        ...
        for entity_type in entities_to_sync:
            ...
            # Aggregate results
            sync_results['results']['total_records_processed'] += entity_result['processed']
            sync_results['results']['records_created'] += entity_result['created']
            sync_results['results']['records_updated'] += entity_result['updated']
            sync_results['results']['records_deleted'] += entity_result.get('deleted', 0)
            sync_results['results']['records_skipped'] += entity_result.get('skipped', 0)
    ...
    def activate_data_integration(self, integration_id: int) -> EntityDict:
        ...
        return self.update_by_id(integration_id, {
            'isActive': True,
            'status': 'active',
    ...
    def bulk_sync_integrations(
        ...
        # Aggregate results
        total_records = sum(
            r.get('results', {}).get('total_records_processed', 0) 
            for r in sync_results
        )
        successful_syncs = len([r for r in sync_results if r.get('sync_status') != 'failed'])
