...
class AllocationCodesEntity(BaseEntity):
    ...
    def get_active_allocation_codes(
        ...
        if allocation_type:
            filters.append({'field': 'AllocationType', 'op': 'eq', 'value': allocation_type})
        if department_id:
            filters.append({'field': 'DepartmentID', 'op': 'eq', 'value': department_id})
    ...
    def get_allocation_code_usage(self, code_id: int) -> Dict[str, Any]:
        ...
        unique_projects = len(set(entry.get('ProjectID') for entry in time_entries if entry.get('ProjectID')))
        
        # Get date range
        dates = [entry.get('DateWorked') for entry in time_entries if entry.get('DateWorked')]
        ...
        return {
            ...
            'last_used': last_used,
            'utilization_percentage': round((billable_hours / total_hours * 100) if total_hours > 0 else 0, 2)
        }
        ...
        for i in range(0, len(code_ids), batch_size):
            ...
            for code_id in batch:
                try:
                    ...
                    self.logger.error(f"Failed to activate allocation code {code_id}: {e}")
                    continue
        ...
        for i in range(0, len(code_ids), batch_size):
            ...
            for code_id in batch:
                try:
                    ...
                    self.logger.error(f"Failed to deactivate allocation code {code_id}: {e}")
                    continue
    ...
    def get_unused_allocation_codes(
        ...
        for code in all_codes:
            ...
            try:
                ...
                self.logger.error(f"Error checking usage for allocation code {code_id}: {e}")
                continue
    ...
    def validate_allocation_code_usage(self, code_id: int) -> Dict[str, Any]:
        ...
        # Check utilization
        if usage_stats['utilization_percentage'] < 50 and usage_stats['total_hours'] > 0:
            warnings.append(f'Low billable utilization: {usage_stats["utilization_percentage"]}%')
