...
class DataExportEntity(BaseEntity):
    ...
    def get_export_jobs_by_status(
        ...
        if created_after:
            filters.append({"field": "created_date", "op": "gte", "value": created_after})
    ...
    def export_filtered_data(
        ...
        filename = output_filename or f"{entity_type}_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{export_format}"
            ...
            try:
                ...
                if entity_result.get("success"):
                    export_results["entity_exports"].append({
                        ...
                        "success": True,
                        "data": entity_result["data"] if not combine_output else None
                    })
                    
                    export_results["total_records"] += entity_result["metadata"]["record_count"]
                    
                    # Add to combined output if requested
                    if combine_output and export_results["combined_data"] is not None:
                        export_results["combined_data"][entity_type] = entity_result["data"]
    ...
    def import_data(
        ...
        for i in range(0, len(parsed_data), batch_size):
            ...
            try:
                ...
                successful_count = len([r for r in batch_result if hasattr(r, 'item_id')])
        ...
        import_results["success_rate"] = round(
            (import_results["successful_imports"] / import_results["total_records"] * 100), 2
        ) if import_results["total_records"] > 0 else 0
    ...
    def transform_data(
            ...
            try:
                ...
                for filter_field, filter_condition in filters.items():
                    if filter_field in transformed_record:
                        ...
                        if not self._evaluate_filter_condition(field_value, filter_condition):
        ...
        transformation_result["transformed_record_count"] = len(transformation_result["transformed_data"])
        transformation_result["success"] = len(transformation_result["transformation_errors"]) == 0
        ...
        for field, stats in field_stats.items():
            validation_result["field_statistics"][field] = {
                ...
                "completeness_percentage": round(
                    (stats["non_null_count"] / len(data) * 100), 2
            ...
            try:
                ...
                if export_result.get("success"):
                    ...
                    bulk_result["export_results"].append({
                        ...
                        "file_size_estimate": len(str(export_result.get("data", "")))
                    })
    ...
    def analyze_export_performance(
        ...
        if export_job_ids:
            filters.append({"field": "id", "op": "in", "value": [str(id) for id in export_job_ids]})
        ...
        processing_times = [Decimal('120.5'), Decimal('89.3'), Decimal('156.7')]  # Mock processing times
        ...
        for entity_type in entity_types:
            try:
                ...
                if integrity_check["issues"]:
                    ...
                    critical_count = len([i for i in integrity_check["issues"] if i.get("severity") == "critical"])
                    warning_count = len([i for i in integrity_check["issues"] if i.get("severity") == "warning"])
                    
                    # Adjust health score based on issues
                    health_score = 100 - (critical_count * 10) - (warning_count * 2)
                    integrity_check["health_score"] = max(Decimal(str(health_score)), Decimal('0'))
    ...
    def _parse_import_data(self, data: Union[str, List[Dict[str, Any]]], format_type: str) -> List[Dict[str, Any]]:
        """Parse import data based on format."""
