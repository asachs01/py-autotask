...
class NotificationRulesEntity(BaseEntity):
    ...
    def create_notification_rule(
        ...
        **kwargs
        ...
        rule_data = {
            ...
            "messageTemplate": message_template or "Entity {entityType} #{id} has been {action}",
    ...
    def configure_triggers(
        ...
        if not validation_result["is_valid"]:
            raise ValueError(f"Invalid trigger configuration: {validation_result['errors']}")
    ...
    def clone_notification_rule(
        ...
        clone_data = {k: v for k, v in source_rule.items() 
    ...
    def get_notification_delivery_history(
        ...
        if recipient_filter:
            filtered_history = [h for h in filtered_history if recipient_filter in h["recipient"]]
    ...
    def get_notification_rules_summary(
        ...
        if include_performance:
            ...
            successful_deliveries = len([d for d in delivery_history if d["status"] == "delivered"])
            failed_deliveries = len([d for d in delivery_history if d["status"] == "failed"])
            ...
            performance_metrics = {
                ...
                "most_used_channel": max(by_channel.items(), key=lambda x: x[1])[0] if by_channel else None
            }
    ...
    def analyze_notification_effectiveness(
        ...
        for channel, stats in channel_performance.items():
            stats["success_rate"] = (stats["delivered"] / stats["total"]) * 100 if stats["total"] > 0 else 0
        ...
        if channel_performance:
            best_channel = max(channel_performance.items(), key=lambda x: x[1]["success_rate"])
            worst_channel = min(channel_performance.items(), key=lambda x: x[1]["success_rate"])
    ...
    def _validate_trigger_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        valid_trigger_types = ["OnCreate", "OnUpdate", "OnDelete", "Scheduled", "Conditional"]
        ...
        if trigger_type == "Scheduled":
            ...
            if not schedule:
                ...
                if schedule.get("frequency") not in valid_frequencies:
                    errors.append(f"Invalid schedule frequency: {schedule.get('frequency')}")
        ...
        if "digest_mode" in config:
            ...
            if isinstance(digest, dict) and digest.get("enabled"):
                ...
                if digest.get("frequency") not in valid_frequencies:
                    errors.append(f"Invalid digest frequency: {digest.get('frequency')}")
