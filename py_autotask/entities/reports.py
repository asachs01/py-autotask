...
class ReportsEntity(BaseEntity):
    ...
    def create_report(
        ...
        **kwargs
        ...
        if not validation_result["is_valid"]:
            raise ValueError(f"Invalid report definition: {validation_result['errors']}")
    ...
    def generate_report(
        ...
        if async_execution:
            ...
            return {
                ...
                "estimated_completion": (execution_start + timedelta(minutes=5)).isoformat(),
                "format": format_type,
    ...
    def clone_report(
        ...
        clone_data = {k: v for k, v in source_report.items() 
                     if k not in ["id", "createDate", "createdByResourceID", "lastModifiedDate", "reportId"]}
    ...
    def aggregate_report_data(
        ...
        aggregated_results["summary"] = {
            ...
            "total_records_processed": base_data.get("metadata", {}).get("total_rows", 0),
    ...
    def export_report(
        ...
        """
        report_data = self.generate_report(report_id, parameters=parameters, format_type=export_format)
    ...
    def analyze_report_performance(
        ...
        if report_id:
            ...
            analysis_data.update({
                ...
                "top_performing_reports": [
                    {"report_id": 1, "name": "Daily Sales Report", "execution_count": 234, "avg_time": 1.2},
                    {"report_id": 2, "name": "Monthly Revenue", "execution_count": 189, "avg_time": 2.1},
                    {"report_id": 3, "name": "Ticket Analysis", "execution_count": 167, "avg_time": 3.8}
                ],
                "performance_issues": [
                    {"report_id": 45, "name": "Complex Analytics", "issue": "Long execution time", "avg_time": 15.2},
                    {"report_id": 78, "name": "Legacy Report", "issue": "High failure rate", "failure_rate": 12.3}
                ]
            })
        ...
        if report_id:
            if analysis_data["performance_metrics"]["average_execution_time"] > 5.0:
                analysis_data["recommendations"].append("Consider optimizing query or adding indexes")
            if analysis_data["performance_metrics"]["success_rate"] < 95.0:
                analysis_data["recommendations"].append("Review error logs and improve error handling")
    ...
    def get_reports_summary(
        ...
        filters = []
        if category_filter:
            filters.append({"field": "reportType", "op": "eq", "value": category_filter})
        if active_only:
            filters.append({"field": "isActive", "op": "eq", "value": True})
    ...
    def monitor_report_system(
        ...
        issues = []
        if check_schedules and monitoring_result["schedule_status"]["failed_schedules_last_24h"] > 5:
            issues.append("High schedule failure rate")
        if check_performance and monitoring_result["performance_status"]["average_response_time"] > 10:
            issues.append("Poor response times")
        if check_errors and monitoring_result["error_status"]["error_rate_percent"] > 5:
            issues.append("High error rate")
        
        if issues:
            monitoring_result["system_status"] = "degraded" if len(issues) < 2 else "critical"
    ...
    def _generate_sample_report_data(
        ...
        return {
            ...
            "rows": [
                {"Date": "2024-01-15", "Category": "Sales", "Amount": 12500.00, "Count": 25},
                {"Date": "2024-01-15", "Category": "Support", "Amount": 3200.00, "Count": 8},
                {"Date": "2024-01-16", "Category": "Sales", "Amount": 15600.00, "Count": 31},
                {"Date": "2024-01-16", "Category": "Support", "Amount": 2800.00, "Count": 7}
            ]
        }
    ...
    def _calculate_next_execution(
        ...
        if frequency == "daily":
            ...
            if now.day == 1:
                ...
                next_exec = now.replace(month=now.month+1, day=1, hour=hour, minute=minute, second=0, microsecond=0)
        else:  # quarterly
            current_quarter = (now.month - 1) // 3 + 1
            next_quarter_month = current_quarter * 3 + 1
            if next_quarter_month > 12:
                next_quarter_month = 1
                next_exec = now.replace(year=now.year+1, month=next_quarter_month, day=1, hour=hour, minute=minute, second=0, microsecond=0)
            else:
                next_exec = now.replace(month=next_quarter_month, day=1, hour=hour, minute=minute, second=0, microsecond=0)
