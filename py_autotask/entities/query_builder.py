...
from ..types import QueryRequest, EntityDict, EntityList
...
class QueryBuilder:
    ...
    def where(
        ...
        if op_str not in ["isNull", "isNotNull"]:
            if value is None:
                raise AutotaskValidationError(f"Value is required for operator '{op_str}'")
    ...
    def where_date_range(
        ...
        self.logger.debug(f"Added date range filter: {field} between {start_date} and {end_date}")
        return self
    ...
    def where_related(
        ...
        related_response = related_query.where(related_field, operator, value).select(["id"]).execute()
        ...
        if not related_response.items:
            ...
            self.logger.debug(f"No matching {related_entity} found for {related_field} {operator} {value}")
        else:
    ...
    def build(self) -> QueryRequest:
        ...
        if self._sort_fields:
            self.logger.warning("Sorting is built but may not be supported by Autotask API")
