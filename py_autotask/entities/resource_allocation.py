...
class ResourceAllocationEntity(BaseEntity):
    ...
    def get_resource_capacity(
        ...
        for allocation in allocations:
            try:
                alloc_start = max(datetime.fromisoformat(start_date).date(),
                                datetime.fromisoformat(allocation['StartDate']).date())
                alloc_end = min(datetime.fromisoformat(end_date).date(),
        ...
        try:
            ...
            utilization_percentage = (total_allocated_hours / available_hours * 100) if available_hours > 0 else 0
    ...
    def find_available_resources(
        ...
        try:
            ...
            resources = resources_response.items if hasattr(resources_response, 'items') else resources_response
        except Exception:
            resources = []
                ...
                try:
                    ...
                    if remaining_hours >= required_total_hours:
                        available_resources.append({
                            'resource_id': resource_id,
                            'resource_name': resource.get('FirstName', '') + ' ' + resource.get('LastName', ''),
    ...
    def get_allocation_conflicts(
        ...
        """
        existing_allocations = self.get_resource_allocations(resource_id, start_date, end_date)
        conflicts = []
            ...
            for allocation_data in batch:
                try:
                    if validate_capacity:
                        ...
                        try:
                            period_start = datetime.fromisoformat(allocation_data['start_date']).date()
                            period_end = datetime.fromisoformat(allocation_data['end_date']).date()
                            days = (period_end - period_start).days + 1
    ...
    def get_team_allocation_summary(
        ...
        for resource_id in resource_ids:
            ...
            team_summary['totals']['available_hours'] += capacity['capacity']['available_hours']
            team_summary['totals']['allocated_hours'] += capacity['capacity']['allocated_hours']
            team_summary['totals']['remaining_hours'] += capacity['capacity']['remaining_hours']
        ...
        if team_summary['totals']['available_hours'] > 0:
            team_summary['totals']['team_utilization_percentage'] = round(
                ...
                team_summary['totals']['available_hours'] * 100, 2
            )
        else:
    ...
    def optimize_resource_allocation(
        ...
        if hours_per_day <= 8.0:
            suitable_resources = [
                r for r in available_resources 
                if r['remaining_hours'] >= required_hours
            ]
            ...
            if suitable_resources:
                scenarios.append({
                    ...
                    'total_resources': 1,
                    'efficiency_score': 100 - suitable_resources[0]['current_utilization']
                })
        ...
        if len(available_resources) >= 2:
            ...
            if remaining_hours <= 0:
                avg_utilization = sum(r['current_utilization'] for r in available_resources[:len(multi_allocation)]) / len(multi_allocation)
                scenarios.append({
