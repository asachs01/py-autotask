...
class ProjectBudgetsEntity(BaseEntity):
    ...
    def get_budget_variance_analysis(
        ...
        actual_costs = self._get_actual_costs(project_id, budget_type, as_of_date, budget)
        ...
        return {
            ...
            'status': status,
            'utilization_percentage': round(float(actual_costs / budget_amount * 100), 2) if budget_amount != 0 else 0,
    ...
    def get_project_budget_summary(
        ...
        for budget in budgets:
            ...
            if budget_id:
                ...
                # Add to totals
                summary['totals']['total_budget'] += Decimal(str(analysis['budget_amount']))
                summary['totals']['total_actual'] += Decimal(str(analysis['actual_costs']))
                summary['totals']['total_variance'] += Decimal(str(analysis['variance_amount']))
        ...
        if summary['totals']['total_budget'] != 0:
            summary['totals']['overall_variance_percentage'] = round(
                summary['totals']['total_variance'] / summary['totals']['total_budget'] * 100, 2
            )
        else:
    ...
    def forecast_budget_completion(
        ...
        try:
            ...
            projects = project_response.items if hasattr(project_response, 'items') else project_response
        ...
        if forecast_method == "linear":
            forecast_data = self._linear_forecast(budget, actual_costs, budget_amount, project)
        elif forecast_method == "weighted":
            forecast_data = self._weighted_forecast(budget, actual_costs, budget_amount, project)
        else:
            forecast_data = self._linear_forecast(budget, actual_costs, budget_amount, project)
        ...
        for budget in budgets:
            ...
            if alert_level:
                alerts.append({
                    ...
                    'message': self._generate_alert_message(alert_level, utilization, analysis)
                })
        ...
        for i in range(0, len(budget_updates), batch_size):
            ...
            for update in batch:
                try:
                    ...
                    self.logger.error(f"Failed to update budget {update.get('id')}: {e}")
                    continue
    ...
    def clone_project_budget(
        ...
        clone_data = {
            ...
            'BudgetAmount': str(adjust_amount) if adjust_amount else original.get('BudgetAmount'),
        ...
        try:
            if budget_type in ['total', 'labor']:
                ...
                if budget.get('StartDate'):
                    time_filters.append({'field': 'DateWorked', 'op': 'gte', 'value': budget['StartDate']})
            ...
            if budget_type in ['total', 'expenses']:
                ...
                if budget.get('StartDate'):
                    expense_filters.append({'field': 'ExpenseDate', 'op': 'gte', 'value': budget['StartDate']})
                
                expense_response = self.client.query('Expenses', expense_filters)
                expenses = expense_response.items if hasattr(expense_response, 'items') else expense_response
    ...
    def _calculate_burn_rate(
        ...
        try:
            ...
            if daily_burn_rate > 0:
                ...
                projected_completion_date = start_date + timedelta(days=int(days_to_completion))
            else:
    ...
    def _linear_forecast(
        ...
        """
        try:
            start_date = datetime.fromisoformat(budget.get('StartDate', datetime.now().isoformat())).date()
            end_date = datetime.fromisoformat(budget.get('EndDate', project.get('EndDate', datetime.now().isoformat()))).date()
