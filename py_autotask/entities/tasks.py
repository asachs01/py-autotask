...
class TasksEntity(BaseEntity):
    ...
    def create(self, task_data: Dict[str, Any]) -> CreateResponse:
        ...
        """
        required_fields = ['title', 'description', 'projectID', 'phaseID', 'assignedResourceID', 'estimatedHours']
    ...
    def get_by_resource(
        ...
        if date_range:
            ...
            filters.extend([
                QueryFilter(field='startDate', op='gte', value=start_date.isoformat() if hasattr(start_date, 'isoformat') else start_date),
                QueryFilter(field='startDate', op='lte', value=end_date.isoformat() if hasattr(end_date, 'isoformat') else end_date)
            ])
        ...
        if status_filter and status_filter.lower() == 'overdue':
            ...
            for task in tasks:
                ...
                if end_date:
                    try:
                        task_end = datetime.fromisoformat(end_date.replace('Z', '+00:00'))
                        if task_end < now:
    ...
    def get_overdue_tasks(self, project_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def start_task(self, task_id: int, start_note: Optional[str] = None) -> UpdateResponse:
        """
    ...
    def get_task_analytics(
        ...
        status_map = {1: 'new', 2: 'in_progress', 3: 'waiting', 5: 'complete', 4: 'cancelled'}
            ...
            if status_id == 5:  # Complete
                ...
                if start_date and completion_date:
                    try:
                        ...
                        completion_time = (completed - start).total_seconds() / 3600  # Hours
        ...
        if analytics['total_tasks'] > 0:
            analytics['completion_rate'] = (completed_tasks / analytics['total_tasks']) * 100
        
        if estimated_hours:
            analytics['avg_estimated_hours'] = sum(estimated_hours) / len(estimated_hours)
        ...
        if completed_tasks > 0:
            analytics['on_time_completion_rate'] = (on_time_completions / completed_tasks) * 100
    ...
    def get_resource_workload(
        ...
        if workload['total_estimated_hours'] > 0:
            weeks_of_work = workload['total_estimated_hours'] / workload['capacity_analysis']['hours_per_week']
            ...
            if weeks_of_work <= 4:  # Next 4 weeks
                workload['capacity_analysis']['capacity_utilization'] = (weeks_of_work / 4) * 100
            else:
                workload['capacity_analysis']['capacity_utilization'] = 100  # Over capacity
        ...
        if estimated_hours is not None:
            try:
                ...
                if est_val <= 0:
                    ...
                    warnings.append("Estimated hours seems unusually high (over 1000 hours)")
