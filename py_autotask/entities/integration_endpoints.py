...
from ..types import EntityDict, QueryFilter, CreateResponse
...
class IntegrationEndpointsEntity(BaseEntity):
    ...
    def get_integration_endpoints_by_type(
        ...
        if active_only:
            filters.append({'field': 'IsActive', 'op': 'eq', 'value': True})
        if health_status:
            filters.append({'field': 'HealthStatus', 'op': 'eq', 'value': health_status})
    ...
    def activate_integration_endpoint(
        ...
        if perform_health_check:
            ...
            if health_result.get('status') != 'HEALTHY':
                raise ValueError(f"Cannot activate unhealthy endpoint: {health_result.get('issues')}")
    ...
    def manage_integrations(
        ...
        if endpoint_ids:
            ...
            filters = []
            if integration_type:
                filters.append({'field': 'IntegrationType', 'op': 'eq', 'value': integration_type})
            ...
            for endpoint in batch:
                ...
                try:
                    if operation == 'START':
                        ...
                        self.deactivate_integration_endpoint(endpoint_id, graceful_shutdown=True)
                        result = self.activate_integration_endpoint(endpoint_id)
                    elif operation == 'SYNC':
                    ...
                    self.logger.error(f"Failed to {operation} endpoint {endpoint_id}: {e}")
        
        management_results['success_count'] = len(management_results['successful_operations'])
        management_results['failure_count'] = len(management_results['failed_operations'])
        
        self.logger.info(f"Integration management completed: {operation} - {management_results['success_count']}/{len(endpoints)} successful")
    ...
    def monitor_endpoint_health(
        ...
        self.logger.info(f"Health monitoring completed: {healthy_count}/{total_endpoints} endpoints healthy")
        ...
        for i in range(0, len(endpoint_ids), batch_size):
            ...
            for endpoint_id in batch:
                try:
                    ...
                    # Activate endpoint
                    result = self.activate_integration_endpoint(endpoint_id, perform_health_check=False)
                    ...
                    self.logger.error(f"Failed to activate integration endpoint {endpoint_id}: {e}")
        
        activation_results['success_count'] = len(activation_results['successful_activations'])
        activation_results['failure_count'] = len(activation_results['failed_activations'])
        activation_results['health_failure_count'] = len(activation_results['health_check_failures'])
        
        self.logger.info(f"Bulk activation completed: {activation_results['success_count']}/{len(endpoint_ids)} endpoints activated")
        ...
        for i in range(0, len(endpoint_ids), batch_size):
            ...
            for endpoint_id in batch:
                try:
                    ...
                    self.logger.error(f"Failed to deactivate integration endpoint {endpoint_id}: {e}")
        
        deactivation_results['success_count'] = len(deactivation_results['successful_deactivations'])
        deactivation_results['failure_count'] = len(deactivation_results['failed_deactivations'])
        
        self.logger.info(f"Bulk deactivation completed: {deactivation_results['success_count']}/{len(endpoint_ids)} endpoints deactivated")
    ...
    def configure_endpoint_security(
        ...
        enhanced_security = {
            'SecuritySettings': {
                ...
                'DataClassification': security_config.get('data_classification', 'INTERNAL'),
                ...
                'CertificateValidation': security_config.get('certificate_validation', True),
    ...
    def validate_integration_endpoint(self, endpoint_id: int) -> Dict[str, Any]:
        ...
        if auth_config:
            ...
            if not auth_type:
                ...
                if not auth_config.get('client_secret') and not auth_config.get('token'):
                    warnings.append('OAuth2 requires either client_secret or existing token')
