...
class ChangeRequestsEntity(BaseEntity):
    ...
    def get_pending_change_requests(
        self,
        assigned_to_resource_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_change_metrics(
        ...
        implemented_changes = len([c for c in changes if c.get("status") == "Implemented"])
        ...
        return {
            ...
            "metrics": {
                ...
                "approval_rate": (approved_changes / total_changes * 100) if total_changes > 0 else 0,
                "implementation_rate": (implemented_changes / approved_changes * 100) if approved_changes > 0 else 0
            }
        }
    
    def bulk_approve_changes(
        self,
        change_approvals: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for approval in change_approvals:
            ...
            try:
                result = self.approve_change_request(change_request_id, approver_resource_id, approval_comments)
                results.append({"id": change_request_id, "success": True, "result": result})
            except Exception as e:
                results.append({"id": change_request_id, "success": False, "error": str(e)})
