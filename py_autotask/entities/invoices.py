...
class InvoicesEntity(BaseEntity):
    ...
    def get_overdue_invoices(self, as_of_date: Optional[date] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_unpaid_invoices(self, account_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        Get unpaid invoices.
    ...
    def mark_as_paid(self, invoice_id: int, payment_date: Optional[date] = None,
                     payment_method: Optional[str] = None) -> UpdateResponse:
        """
    ...
    def void_invoice(self, invoice_id: int, void_reason: Optional[str] = None) -> UpdateResponse:
        """
        Void an invoice.
    ...
    def get_invoice_aging_report(self, account_id: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    def get_invoice_summary(self, account_id: Optional[int] = None,
        ...
        if account_id:
            overdue_invoices = [inv for inv in overdue_invoices if inv.get('accountID') == account_id]
        ...
        if due_date:
            try:
                if isinstance(due_date, str):
                    ...
                    if invoice_date:
                        invoice_date_obj = datetime.fromisoformat(invoice_date.replace('Z', '+00:00')).date()
