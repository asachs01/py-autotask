...
class VendorTypesEntity(BaseEntity):
    ...
    def get_vendor_type_statistics(
        self,
        vendor_type_id: int
    ) -> Dict[str, Any]:
        """
    ...
    def clone_vendor_type(
        ...
        clone_data = {k: v for k, v in source_vendor_type.items() 
    ...
    def get_vendor_types_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_category": {category: len(types) for category, types in category_groups.items()},
            "by_payment_terms": {terms: len(types) for terms, types in payment_terms_groups.items()},
    ...
    def bulk_update_payment_terms(
        self,
        payment_terms_updates: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for update in payment_terms_updates:
            ...
            try:
                ...
                results.append({"id": vendor_type_id, "success": True, "result": result})
            except Exception as e:
                results.append({"id": vendor_type_id, "success": False, "error": str(e)})
