...
class ProjectMilestonesEntity(BaseEntity):
    ...
    def clone_milestone(
        ...
        if adjust_dates and original.get('DueDate'):
            try:
                ...
                clone_data['DueDate'] = (datetime.now() + timedelta(days=30)).isoformat()
        ...
        for i in range(0, len(milestone_updates), batch_size):
            ...
            for update in batch:
                try:
                    ...
                    self.logger.error(f"Failed to update milestone {update.get('id')}: {e}")
                    continue
    ...
    def get_milestone_dependencies(self, milestone_id: int) -> Dict[str, Any]:
        ...
        dependent_filters = [{'field': 'PredecessorMilestoneID', 'op': 'eq', 'value': milestone_id}]
        dependent_response = self.query(filters=dependent_filters)
        dependent_milestones = dependent_response.items if hasattr(dependent_response, 'items') else dependent_response
