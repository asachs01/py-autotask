...
class WorkflowRulesEntity(BaseEntity):
    ...
    def activate_workflow_rule(
        ...
        """
        return self.update_by_id(rule_id, {
            "isActive": True,
    ...
    def bulk_activate_workflow_rules(
        self,
        rule_ids: List[int]
    ) -> Dict[str, Any]:
        """
    ...
    def create_workflow_template(
        ...
        for i, rule_def in enumerate(rule_definitions):
            ...
            if not validation["is_valid"]:
                validation_errors.extend([f"Rule {i+1}: {error}" for error in validation["errors"]])
    ...
    def apply_workflow_template(
        ...
        for rule_def in rule_definitions:
            try:
                ...
                results.append({"rule_name": rule_def.get("name"), "success": True, "result": result})
            except Exception as e:
                results.append({"rule_name": rule_def.get("name"), "success": False, "error": str(e)})
