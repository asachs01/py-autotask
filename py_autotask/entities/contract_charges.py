...
class ContractChargesEntity(BaseEntity):
    ...
    def get_revenue_by_contract(
        ...
        if contract_ids:
            contract_filter = " or ".join([f"contractID eq {cid}" for cid in contract_ids])
        ...
        for contract_data in revenue_by_contract.values():
            ...
            contract_data["profit_percentage"] = float(profit / contract_data["total_revenue"] * 100) if contract_data["total_revenue"] > 0 else 0
        
        total_revenue = sum(data["total_revenue"] for data in revenue_by_contract.values())
        ...
        return {
            ...
            "summary": {
                ...
                "total_charges": sum(data["charge_count"] for data in revenue_by_contract.values()),
