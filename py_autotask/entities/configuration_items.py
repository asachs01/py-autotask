...
class ConfigurationItemsEntity(BaseEntity):
    ...
    def create(self, ci_data: Dict[str, Any]) -> CreateResponse:
        ...
        """
        required_fields = ['accountID', 'configurationItemName', 'configurationItemCategoryID', 'configurationItemType']
    ...
    def get_by_account(
        ...
        if ci_type is not None:
            filters.append(QueryFilter(field='configurationItemType', op='eq', value=ci_type))
    ...
    def get_expiring_warranties(
        ...
        filters = [
            QueryFilter(field='warrantyExpirationDate', op='lte', value=future_date.isoformat()),
            QueryFilter(field='warrantyExpirationDate', op='gte', value=datetime.now().isoformat()),
            QueryFilter(field='isActive', op='eq', value=True)
        ]
    ...
    def get_by_location(self, location: str, account_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_ci_relationships(self, ci_id: int) -> Dict[str, Any]:
        ...
        # Get related tickets
        ticket_filters = [QueryFilter(field='configurationItemID', op='eq', value=ci_id)]
        relationships['tickets'] = self.client.query('Tickets', filters=ticket_filters)
        
        # Get related contracts
        contract_filters = [QueryFilter(field='configurationItemID', op='eq', value=ci_id)]
        relationships['contracts'] = self.client.query('Contracts', filters=contract_filters)
        
        # Get related time entries
        time_filters = [QueryFilter(field='configurationItemID', op='eq', value=ci_id)]
        relationships['time_entries'] = self.client.query('TimeEntries', filters=time_filters)
        ...
        if ci_data:
            ...
            # Get child CIs
            child_filters = [QueryFilter(field='parentConfigurationItemID', op='eq', value=ci_id)]
    ...
    def update_warranty_info(
        ...
        """
        update_data = {
            'warrantyExpirationDate': warranty_expiration.isoformat() if hasattr(warranty_expiration, 'isoformat') else warranty_expiration
        }
    ...
    def deactivate_ci(self, ci_id: int, deactivation_reason: Optional[str] = None) -> UpdateResponse:
        """
    ...
    def get_ci_inventory_report(self, account_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        for ci in cis:
            ...
            report['by_manufacturer'][manufacturer] = report['by_manufacturer'].get(manufacturer, 0) + 1
            ...
            if warranty_date:
                try:
                    warranty_exp = datetime.fromisoformat(warranty_date.replace('Z', '+00:00'))
            ...
            if install_date:
                try:
                    installed = datetime.fromisoformat(install_date.replace('Z', '+00:00'))
    ...
    def search_configuration_items(self, search_criteria: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        if text_search:
            ...
            filters.append(QueryFilter(field='configurationItemName', op='contains', value=text_search))
        
        # Specific field filters
        if search_criteria.get('manufacturer'):
            filters.append(QueryFilter(field='manufacturer', op='eq', value=search_criteria['manufacturer']))
        
        if search_criteria.get('ci_type'):
            filters.append(QueryFilter(field='configurationItemType', op='eq', value=search_criteria['ci_type']))
        
        if search_criteria.get('location'):
            filters.append(QueryFilter(field='location', op='contains', value=search_criteria['location']))
        
        if search_criteria.get('account_id'):
            filters.append(QueryFilter(field='accountID', op='eq', value=search_criteria['account_id']))
        ...
        if serial_number:
            ...
            if existing_cis and len(existing_cis) > 0:
                ...
                if not ci_id or any(ci['id'] != ci_id for ci in existing_cis):
                    warnings.append(f"Serial number '{serial_number}' is already in use")
