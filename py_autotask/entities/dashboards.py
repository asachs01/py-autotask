...
class DashboardsEntity(BaseEntity):
    ...
    def clone_dashboard(
        ...
        if not source_dashboard:
            raise AutotaskValidationError(f"Source dashboard {source_dashboard_id} not found")
        
        # Prepare clone data
        clone_data = {k: v for k, v in source_dashboard.items() 
                     if k not in ["id", "created_date", "last_modified", "widget_count"]}
        
        clone_data.update({
            "name": new_name,
            "description": new_description or f"Clone of {source_dashboard.get('name', '')}",
            "created_date": datetime.now().isoformat(),
            "last_modified": datetime.now().isoformat(),
        ...
        self.logger.info(f"Cloned dashboard {source_dashboard_id} to {cloned_dashboard.item_id}")
    ...
    def configure_widgets(
        ...
        self.update_by_id(dashboard_id, {
    ...
    def analyze_dashboard_usage(
        ...
        usage_data["usage_summary"].update({
            ...
            "most_viewed_widgets": [
                ...
                {"widget_id": "w003", "title": "Resource Utilization", "views": 289}
            ],
    ...
    def get_dashboard_summary(
        ...
        if dashboard_type:
            if isinstance(dashboard_type, DashboardType):
                dashboard_type = dashboard_type.value
            filters.append({"field": "dashboard_type", "op": "eq", "value": dashboard_type})
        ...
        return {
            ...
            "activation_percentage": round(
                (active_dashboards / total_dashboards * 100) if total_dashboards > 0 else 0, 2
            ),
    ...
    def analyze_dashboard_performance(
        self,
        dashboard_id: int,
        performance_period: str = "last_7_days"
    ) -> Dict[str, Any]:
        """
    ...
    def monitor_dashboard_health(
        ...
        if dashboard_ids:
            ...
            dashboards = self.query(filters={"field": "is_active", "op": "eq", "value": True}).items
    ...
    def _clone_dashboard_widgets(
        ...
        self.logger.info(f"Cloning widgets from {source_dashboard_id} to {target_dashboard_id}")
    
    def _export_dashboard_widgets(
        self,
        dashboard_id: int
    ) -> List[Dict[str, Any]]:
    ...
    def _get_recent_dashboard_activity(
        self,
        dashboards: List[EntityDict]
    ) -> Dict[str, Any]:
        """Get recent activity summary for dashboards."""
        return {
            "recently_created": len([d for d in dashboards if self._is_recent(d.get("created_date"))]),
            "recently_modified": len([d for d in dashboards if self._is_recent(d.get("last_modified"))]),
