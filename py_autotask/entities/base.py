...
class BaseEntity:
    ...
    def query_all(
        ...
        while page_count < max_pages:
            try:
                ...
                self.logger.error(f"Error during pagination on page {page_count + 1}: {e}")
                if page_count == 0:
    ...
    def update_by_id(self, entity_id: int, update_data: EntityDict) -> Optional[EntityDict]:
        """
    ...
    def batch_create(
        ...
        """
        self.logger.info(f"Starting batch create of {len(entities_data)} {self.entity_name} entities")
        ...
        try:
            ...
            self.logger.info(f"Successfully created {len(results)} {self.entity_name} entities")
            return results
    ...
    def batch_update(
        ...
        """
        self.logger.info(f"Starting batch update of {len(entities_data)} {self.entity_name} entities")
        ...
        try:
            ...
            self.logger.info(f"Successfully updated {len(results)} {self.entity_name} entities")
            return results
    ...
    def batch_delete(
        ...
        """
        self.logger.info(f"Starting batch delete of {len(entity_ids)} {self.entity_name} entities")
        
        try:
            results = self.client.batch_delete(
                self.entity_name, 
                entity_ids, 
                batch_size
            )
            ...
            self.logger.info(f"Successfully deleted {successful_count}/{len(entity_ids)} {self.entity_name} entities")
            return results
    ...
    def batch_get(
        ...
        for i in range(0, len(entity_ids), batch_size):
            ...
            try:
                response = self.query(
                    filters={"field": "id", "op": "in", "value": [str(id) for id in batch]},
                    include_fields=include_fields
                )
