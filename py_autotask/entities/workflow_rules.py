...
class WorkflowRulesEntity(BaseEntity):
    ...
    def get_active_workflow_rules(
        ...
        if trigger_event:
            filters.append({'field': 'TriggerEvent', 'op': 'eq', 'value': trigger_event})
        ...
        for i in range(0, len(rule_ids), batch_size):
            ...
            for rule_id in batch:
                try:
                    ...
                    self.logger.error(f"Failed to activate workflow rule {rule_id}: {e}")
                    continue
        ...
        for i in range(0, len(rule_ids), batch_size):
            ...
            for rule_id in batch:
                try:
                    ...
                    self.logger.error(f"Failed to deactivate workflow rule {rule_id}: {e}")
                    continue
    ...
    def get_entity_type_distribution(self) -> Dict[str, Any]:
        ...
        for rule in all_rules:
            ...
            if rule.get('IsActive'):
                active_entity_counts[entity_type] = active_entity_counts.get(entity_type, 0) + 1
