...
class ResourceSkillsEntity(BaseEntity):
    ...
    def find_resources_by_skills(
        ...
        for req in required_skills:
            ...
            skill_filters.append(f"(skillID eq {skill_id} and skillLevel ge {min_level})")
    ...
    def bulk_update_skills(
        ...
        for update in skill_updates:
            ...
            try:
                ...
                results.append({"id": resource_skill_id, "success": True, "result": result})
            except Exception as e:
                results.append({"id": resource_skill_id, "success": False, "error": str(e)})
    ...
    def get_skill_development_plan(
        ...
        if target_skills:
            for target in target_skills:
                ...
                # Find current level
                current_skill = next((s for s in current_skills if s.get("skillID") == skill_id), None)
                current_level = current_skill.get("skillLevel", 0) if current_skill else 0
