...
class BillingItemsEntity(BaseEntity):
    ...
    def get_unbilled_items_summary(
        ...
        for item in items:
            if item.get("unitPrice") and item.get("quantity"):
                total_amount += Decimal(str(item["unitPrice"])) * Decimal(str(item["quantity"]))
            if item.get("unitCost") and item.get("quantity"):
                total_cost += Decimal(str(item["unitCost"])) * Decimal(str(item["quantity"]))
        ...
        return {
            ...
            "profit_percentage": float((total_amount - total_cost) / total_amount * 100) if total_amount > 0 else 0
        }
    ...
    def get_revenue_by_billing_code(
        ...
        if billing_code_ids:
            code_filter = " or ".join([f"billingCodeID eq {code_id}" for code_id in billing_code_ids])
        ...
        for item in items:
            ...
            if billing_code_id:
                ...
                if item.get("unitPrice") and item.get("quantity"):
                    revenue = Decimal(str(item["unitPrice"])) * Decimal(str(item["quantity"]))
                    code_data["total_revenue"] += revenue
                    
                if item.get("unitCost") and item.get("quantity"):
                    cost = Decimal(str(item["unitCost"])) * Decimal(str(item["quantity"]))
        ...
        for code_data in revenue_by_code.values():
            ...
            code_data["profit_percentage"] = float(profit / code_data["total_revenue"] * 100) if code_data["total_revenue"] > 0 else 0
        ...
        return {
            ...
            "summary": {
                ...
                "total_items": sum(data["item_count"] for data in revenue_by_code.values()),
                "total_revenue": total_revenue,
                "total_cost": total_cost,
                "total_profit": total_revenue - total_cost,
                "overall_margin_percentage": float((total_revenue - total_cost) / total_revenue * 100) if total_revenue > 0 else 0
            },
