"""
TicketNoteAttachments entity for Autotask API operations.
"""
...
class TicketNoteAttachmentsEntity(BaseEntity):
    """
    Handles all Ticket Note Attachment-related operations for the Autotask API.

    Ticket Note Attachments in Autotask represent files, documents, and other
    attachments associated with ticket note records.
    """
    ...
    def create_note_attachment(
        ...
        **kwargs,
        ...
        """
        Create a new attachment for a ticket note.

        Args:
            note_id: ID of the ticket note
            attachment_data: Binary data of the attachment (base64 encoded)
        ...
        if title:
            attachment_data_dict["Title"] = title
        if attach_date:
            attachment_data_dict["AttachDate"] = attach_date

        return self.create(attachment_data_dict)

    def get_note_attachments(
        self, note_id: int, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        Get all attachments for a specific ticket note.
        ...
        """
        filters = [QueryFilter(field="TicketNoteID", op="eq", value=note_id)]

        response = self.query(filters=filters, max_records=limit)
        return response.items

    def get_ticket_note_attachments(
        self, ticket_id: int, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        Get all note attachments for a specific ticket.
        ...
        Returns:
            List of all note attachments for the ticket
        """
        # First get all ticket notes
        notes_response = self.client.query(
            "TicketNotes",
            filters=[QueryFilter(field="TicketID", op="eq", value=ticket_id)],
        )
        ticket_notes = notes_response.get("items", [])

        if not ticket_notes:
            return []

        note_ids = [note["id"] for note in ticket_notes]

        # Get attachments for all these notes
        filters = [QueryFilter(field="TicketNoteID", op="in", value=note_ids)]
        response = self.query(filters=filters, max_records=limit)
        return response.items
    ...
    def get_attachments_by_type(
        ...
        """
        filters = [
            QueryFilter(field="TicketNoteID", op="eq", value=note_id),
    ...
    def get_image_attachments(
        ...
        """
        Get all image attachments for a ticket note.
    ...
    def get_document_attachments(
        ...
        """
        Get all document attachments for a ticket note.
