...
class SystemHealthEntity(BaseEntity):
    ...
    def get_system_health_status(
        ...
        filters = []
        if severity_filter:
            filters.append({"field": "currentStatus", "op": "eq", "value": severity_filter})
        ...
        self.logger.info(f"Retrieved system health status: {health_summary['overall_status']}")
    ...
    def diagnose_system_issues(
        ...
        if hasattr(monitors, 'items'):
            ...
            for monitor in monitors.items:
                ...
                if current_status == "Critical":
                    ...
                    diagnostic_report["issues_found"]["informational"].append(issue_data)
                    
                # Generate recommendations based on monitor type and status
                if current_status in ["Warning", "Critical"]:
                    recommendations = self._generate_recommendations(monitor_type, current_status, issue_data)
        ...
        diagnostic_report["summary"]["recommended_actions"] = len(diagnostic_report["recommendations"])
        
        self.logger.info(f"System diagnostic completed: {diagnostic_report['summary']['total_issues']} issues found")
    ...
    def monitor_system_health(
        ...
        self.logger.info(f"Health monitoring session completed: {monitoring_session['session_id']}")
    ...
    def optimize_system_performance(
        ...
        optimization_report = {
            ...
            "areas_analyzed": optimization_areas or ["CPU", "Memory", "Disk", "Network", "Database", "API"],
            "current_performance": {},
        ...
        for area in optimization_report["areas_analyzed"]:
            ...
            recommendations = self._generate_optimization_recommendations(area, area_analysis)
        ...
        if apply_recommendations:
            safe_optimizations = [rec for rec in optimization_report["recommendations"]["immediate"] 
        ...
        self.logger.info(f"System optimization analysis completed: {total_recommendations} recommendations generated")
        return optimization_report
    
    def get_health_summary(
        self,
        summary_type: str = "current",
    ...
    def get_performance_metrics(
        ...
        self.logger.info(f"Retrieved performance metrics for {len(metrics['metric_types'])} types")
        return metrics
    ...
    def generate_health_report(
        ...
        report = {
            ...
            "appendix": {
                ...
                "data_sources": ["System monitors", "Performance counters", "Error logs"],
        ...
        current_health = self.get_system_health_status(include_details=True)
        report["executive_summary"]["critical_issues"] = current_health["status_counts"]["critical"]
        ...
        if include_recommendations:
            optimization_analysis = self.optimize_system_performance(analysis_period_days=7)
            report["recommendations"] = optimization_analysis["recommendations"]["immediate"]
        ...
        if not report["executive_summary"]["key_findings"]:
            report["executive_summary"]["key_findings"].append("System operating within normal parameters")
        
        self.logger.info(f"Generated {report_type} health report: {report['report_id']}")
        return report
    ...
    def _analyze_optimization_area(
        ...
        if area == "CPU":
            analysis.update({
                ...
                "bottlenecks_identified": ["High process scheduling overhead", "Inefficient algorithms"]
            })
        elif area == "Memory":
            analysis.update({
                "current_utilization": 72.0,
                "peak_utilization": 88.0,
                "bottlenecks_identified": ["Memory leaks in background processes", "Excessive caching"]
            })
