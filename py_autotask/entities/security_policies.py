...
class SecurityPoliciesEntity(BaseEntity):
    ...
    def get_active_security_policies(
        ...
        if severity_level:
            filters.append({"field": "severityLevel", "op": "eq", "value": severity_level})
        if compliance_framework:
            filters.append({"field": "complianceFramework", "op": "eq", "value": compliance_framework})
    ...
    def secure_policy_encryption(
        ...
        encryption_config = {
            ...
            "next_rotation_date": (datetime.now() + timedelta(days=key_rotation_days)).isoformat(),
    ...
    def validate_security_compliance(
        ...
        compliance_requirements = {
            "SOX": {
                "required_controls": ["access_control", "audit_logging", "data_integrity"],
                "minimum_encryption": "AES-128",
                "audit_frequency": 30,
                "access_review_frequency": 90
            },
            "GDPR": {
                "required_controls": ["data_protection", "consent_management", "breach_notification"],
                "minimum_encryption": "AES-256",
                "data_retention_limits": True,
                "privacy_by_design": True
            },
            "HIPAA": {
                "required_controls": ["access_control", "audit_logging", "data_encryption"],
                "minimum_encryption": "AES-256",
                "audit_frequency": 30,
        ...
        if policy_ids:
            ...
            response = self.get_active_security_policies(compliance_framework=compliance_framework)
            policies = response
        ...
        for policy in policies:
            ...
            if total_policy_checks > 0:
                policy_validation["compliance_score"] = (checks_passed / total_policy_checks) * 100
        ...
        average_compliance_score = overall_compliance_score / len(policies) if policies else 0
        fully_compliant_policies = len([r for r in validation_results if r["is_compliant"]])
    ...
    def audit_security_settings(
        ...
        audit_results = {
            "audit_summary": {
                ...
                "critical_findings": len([f for f in audit_findings if f.get("risk_level") == "Critical"]),
                "high_findings": len([f for f in audit_findings if f.get("risk_level") == "High"])
            },
    ...
    def _check_encryption_compliance(self, policy: Dict[str, Any], min_encryption: str) -> bool:
        ...
        return encryption_levels.index(current_encryption) >= encryption_levels.index(min_encryption)
    
    def _check_audit_frequency(self, policy: Dict[str, Any], required_frequency: int) -> bool:
    ...
    def _generate_compliance_report(
        ...
        return {
            ...
            ][:5],  # Top 5 non-compliant policies
            "recommendations": [
    ...
    def _audit_individual_policy(self, policy: Dict[str, Any]) -> Dict[str, Any]:
        ...
        security_score = ((len(audit_checks) - len(failed_checks)) / len(audit_checks)) * 100
        ...
        return {
            ...
            "risk_level": "Critical" if len(failed_checks) > 2 else "High" if len(failed_checks) > 0 else "Low",
    ...
    def _generate_audit_recommendations(self, findings: List[Dict[str, Any]]) -> List[str]:
        ...
        if critical_count > 0:
            recommendations.append(f"Immediate action required: {critical_count} critical security findings")
        
        high_count = len([f for f in findings if f.get("risk_level") == "High"])
        if high_count > 0:
            recommendations.append(f"Address {high_count} high-risk security issues within 30 days")
    ...
    def _analyze_historical_changes(self, policies: List[Dict[str, Any]], days_back: int) -> Dict[str, Any]:
