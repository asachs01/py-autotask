...
class TaskDependenciesEntity(BaseEntity):
    ...
    def get_task_dependencies(
        self,
        task_id: int,
        direction: str = "both"  # predecessors, successors, both
        ...
        if direction in ['predecessors', 'both']:
            ...
            result['predecessors'] = pred_response.items if hasattr(pred_response, 'items') else pred_response
        
        if direction in ['successors', 'both']:
            # Get tasks that depend on this task
            succ_filters = [{'field': 'PredecessorTaskID', 'op': 'eq', 'value': task_id}]
            succ_response = self.query(filters=succ_filters)
            result['successors'] = succ_response.items if hasattr(succ_response, 'items') else succ_response
    ...
    def get_project_dependency_graph(self, project_id: int) -> Dict[str, Any]:
        ...
        if task_ids:
            ...
            pred_filters = [{'field': 'PredecessorTaskID', 'op': 'in', 'value': [str(tid) for tid in task_ids]}]
            pred_response = self.query(filters=pred_filters)
            project_dependencies.extend(pred_response.items if hasattr(pred_response, 'items') else pred_response)
            
            succ_filters = [{'field': 'SuccessorTaskID', 'op': 'in', 'value': [str(tid) for tid in task_ids]}]
            succ_response = self.query(filters=succ_filters)
            project_dependencies.extend(succ_response.items if hasattr(succ_response, 'items') else succ_response)
        ...
        root_tasks = [
            task_id for task_id in reverse_dependency_map 
            if not reverse_dependency_map[task_id]
        ]
        
        # Find leaf tasks (no successors)
        leaf_tasks = [
            task_id for task_id in dependency_map 
            if not dependency_map[task_id]
        ]
    ...
    def _find_critical_path(
        self,
        tasks: List[EntityDict],
        dependencies: List[EntityDict]
    ) -> Dict[str, Any]:
        """
        ...
        project_duration = max(
            task_lookup[task_id]['earliest_finish'] 
            for task_id in task_lookup
        )
            ...
            if pred_name in task_mapping and succ_name in task_mapping:
                try:
                    result = self.create_task_dependency(
                        ...
                        dependency_type=dep_def.get("dependency_type", "finish_to_start"),
    ...
    def clone_project_dependencies(
        ...
        if not task_mapping:
            ...
            task_mapping = self._generate_task_mapping(source_project_id, target_project_id)
            ...
            for succ_info in successors:
                ...
                try:
                    result = self.create_task_dependency(
                        ...
                        dependency_type=succ_info.get("dependency_type", "finish_to_start"),
    ...
    def get_dependency_impact_analysis(
        ...
        critical_task_ids = {t["task_id"] for t in critical_path.get("critical_tasks", [])}
        
        if task_id in critical_task_ids:
            impact["critical_path_affected"] = True
            impact["schedule_slip_days"] = abs(change_duration) / 8  # Convert hours to days
    ...
    def get_dependency_health_score(
        self,
        project_id: int
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "recommendations": self._generate_health_recommendations(final_score, violations)
        }
        ...
        for update in updates:
            ...
            if dependency_id is not None and new_lag_time is not None:
                try:
                    ...
                    self.logger.error(f"Failed to update lag for dependency {dependency_id}: {e}")
        
        return results
    
    def get_dependency_bottlenecks(
        self,
        project_id: int
    ) -> List[Dict[str, Any]]:
        """
            ...
            if successor_count > 3 or predecessor_count > 3:
                try:
                    ...
                    bottlenecks.append({
                        ...
                        "bottleneck_type": "fan_out" if successor_count > 3 else "fan_in",
                        "risk_level": "high" if max(successor_count, predecessor_count) > 5 else "medium"
                    })
                except Exception:
                    pass
    ...
    def get_dependency_summary_advanced(
        self,
        project_id: int
    ) -> Dict[str, Any]:
        """
        ...
        network_density = (total_deps / (total_tasks * (total_tasks - 1))) * 100 if total_tasks > 1 else 0
        ...
        advanced_summary = {
            **basic_summary,
            ...
            "high_risk_bottlenecks": len([b for b in bottlenecks if b["risk_level"] == "high"]),
            "complexity_index": self._calculate_complexity_index(dependency_graph),
            "parallel_work_potential": self._calculate_parallel_potential(dependency_graph),
    ...
    def _generate_task_mapping(
        ...
        try:
            ...
            for i, (source_task, target_task) in enumerate(zip(source_list, target_list)):
    ...
    def _generate_health_recommendations(
        ...
        if score < 40:
            recommendations.append("Critical dependency issues require immediate attention")
