...
from ..types import EntityDict, AttachmentData
from ..exceptions import AutotaskValidationError, AutotaskConnectionError, AutotaskTimeoutError
...
class AttachmentsEntity(BaseEntity):
    ...
    def upload_file(
        ...
        try:
            with open(file_path, 'rb') as file_data:
                ...
                files = {
                    'file': (file_path.name, file_data, content_type)
                }
    ...
    def batch_upload(
        ...
        for i in range(0, len(file_paths), batch_size):
            ...
            self.logger.info(f"Processing upload batch {batch_num}/{total_batches} ({len(batch)} files)")
        ...
        self.logger.info(f"Batch upload complete: {len(results)}/{len(file_paths)} files uploaded")
        return results 