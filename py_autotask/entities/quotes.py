...
class QuotesEntity(BaseEntity):
    ...
    def get_by_status(self, status: str, account_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        if account_id:
            filters.append(QueryFilter(field="soldToAccountID", op="eq", value=account_id))
    ...
    def get_expired_quotes(self, as_of_date: Optional[date] = None) -> List[Dict[str, Any]]:
        """
        Get expired quotes.
    ...
    def get_expiring_soon(self, days_ahead: int = 7) -> List[Dict[str, Any]]:
        ...
        filters = [
            ...
            QueryFilter(field="expirationDate", op="lte", value=future_date.isoformat()),
    ...
    def accept_quote(self, quote_id: int, accepted_by: Optional[str] = None,
                    acceptance_date: Optional[date] = None) -> UpdateResponse:
        """
        Accept a quote.
    ...
    def reject_quote(self, quote_id: int, rejected_by: Optional[str] = None,
                    rejection_reason: Optional[str] = None,
                    rejection_date: Optional[date] = None) -> UpdateResponse:
        """
        Reject a quote.
    ...
    def extend_expiration(self, quote_id: int, new_expiration_date: date) -> UpdateResponse:
        """
    ...
    def get_quote_conversion_rate(self, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        if stats['total_quotes'] > 0:
            ...
            stats['rejection_rate'] = (stats['rejected_quotes'] / stats['total_quotes']) * 100
        else:
    ...
    def get_quote_summary(self, account_id: Optional[int] = None,
        ...
        if summary['total_quotes'] > 0:
            summary['average_quote_value'] = summary['total_value'] / summary['total_quotes']
        else:
        ...
        if expiration_date:
            try:
                if isinstance(expiration_date, str):
                    exp_date_obj = datetime.fromisoformat(expiration_date.replace('Z', '+00:00')).date()
                    if quote_date:
                        quote_date_obj = datetime.fromisoformat(quote_date.replace('Z', '+00:00')).date()
