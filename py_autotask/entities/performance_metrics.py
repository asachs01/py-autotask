...
from ..types import EntityDict, EntityList, QueryFilter, CreateResponse
...
class PerformanceMetricsEntity(BaseEntity):
    ...
    def __init__(self, client: "AutotaskClient", entity_name: str = "PerformanceMetrics") -> None:
        """
    ...
    def get_metrics_by_type(
        ...
        if source_entity:
            filters.append({"field": "SourceEntity", "op": "eq", "value": source_entity})
    ...
    def get_metrics_by_entity(
        ...
        if entity_id:
            filters.append({"field": "SourceEntityId", "op": "eq", "value": str(entity_id)})
    ...
    def clone_metric_configuration(
        ...
        cloned_metric = {
            ...
            "MonitoringConfig": json.dumps(monitoring_config) if monitoring_config else None,
            "CreatedDateTime": datetime.utcnow().isoformat(),
            "IsActive": True,
    ...
    def get_performance_summary(
        ...
        filters = [
            {"field": "MeasurementTimestamp", "op": "gte", "value": start_time.isoformat()},
            {"field": "IsActive", "op": "eq", "value": True}
        ]
        
        if entity_filter:
            filters.append({"field": "SourceEntity", "op": "eq", "value": entity_filter})
        
        if metric_categories:
            filters.append({"field": "MetricCategory", "op": "in", "value": metric_categories})
        ...
        for category_data in summary["by_category"].values():
            if category_data["metrics"]:
                ...
                category_data["median_value"] = statistics.median(category_data["metrics"])
                if len(category_data["metrics"]) > 1:
                    category_data["std_dev"] = statistics.stdev(category_data["metrics"])
                else:
        ...
        for metric in metrics:
            ...
            summary["by_entity"][entity]["categories"].add(metric.get("MetricCategory", "unknown"))
            summary["by_entity"][entity]["types"].add(metric.get("MetricType", "unknown"))
        ...
        if all_values:
            summary["performance_indicators"] = {
                ...
                "overall_std_dev": statistics.stdev(all_values) if len(all_values) > 1 else 0,
        ...
        if validate_data:
            ...
            for i, metric_data in enumerate(metrics_data):
                try:
                    ...
                    raise AutotaskValidationError(f"Validation failed for metric {i}: {e}")
        ...
        for i in range(0, len(metrics_data), batch_size):
            ...
            for metric_data in batch:
                entity_data = {
                    ...
                    "AdditionalData": json.dumps(metric_data.get("additional_data")) if metric_data.get("additional_data") else None,
                    "CreatedDateTime": datetime.utcnow().isoformat(),
                    "IsActive": True
                }
    ...
    def configure_metric_collection(
        ...
        """
        self.logger.info(f"Configuring metric collection for entity type: {entity_type}")
        ...
        collection_config = {
            ...
            "next_collection": (datetime.utcnow() + timedelta(seconds=collection_frequency)).isoformat()
        }
    ...
    def monitor_system_performance(
        ...
        """
        self.logger.info(f"Starting system performance monitoring for {monitoring_duration}")
        ...
        if real_time_updates:
            snapshot_interval = min(monitoring_duration.total_seconds() / 10, 60)  # Max 1 minute intervals
            ...
            while current_time < end_time:
                snapshot_filters = filters + [
                    {"field": "MeasurementTimestamp", "op": "gte", "value": current_time.isoformat()},
                    {"field": "MeasurementTimestamp", "op": "lt", "value": (current_time + timedelta(seconds=snapshot_interval)).isoformat()}
                ]
                
                snapshot_metrics = self.query_all(filters=snapshot_filters)
                
                if snapshot_metrics:
                    snapshot = self._create_performance_snapshot(snapshot_metrics, current_time)
                    monitoring_results["performance_snapshots"].append(snapshot)
                    
                    # Check for anomalies
                    if alert_on_anomalies:
                        anomalies = self._detect_performance_anomalies(snapshot, baseline_stats)
                        if anomalies:
        ...
        monitoring_results["summary_statistics"] = {
            ...
            "unique_entities_monitored": len(set(m.get("SourceEntity") for m in final_metrics)),
        ...
        for metric_type, type_metrics in metrics_by_type.items():
            ...
            if len(values) >= 2:
                ...
                trend_analysis["trends_by_type"][metric_type] = {
                    ...
                    "period_change": ((values[-1] - values[0]) / values[0] * 100) if values[0] != 0 else 0,
                    "predictions": self._generate_trend_predictions(values, metric_type)
                }
        ...
        for entity, entity_metrics in metrics_by_entity.items():
            ...
            if len(values) >= 2:
                ...
                trend_analysis["trends_by_entity"][entity] = {
                    ...
                    "trend_direction": trend_stats["direction"],
                    "trend_strength": trend_stats["strength"],
                    "performance_score": self._calculate_entity_performance_score(entity_metrics),
                    "recommendations": self._generate_entity_recommendations(entity, trend_stats)
                }
    ...
    def optimize_performance(
        ...
        """
        self.logger.info(f"Starting performance optimization analysis for target: {optimization_target}")
        
        # Build filters for optimization analysis
        filters = [
            {"field": "IsActive", "op": "eq", "value": True},
            {"field": "MeasurementTimestamp", "op": "gte", "value": (datetime.utcnow() - timedelta(days=7)).isoformat()}
        ]
        ...
        performance_analysis = self._analyze_current_performance(metrics, optimization_strategy)
        ...
        optimization_results["optimization_summary"] = {
            ...
            "expected_performance_gain": expected_improvements.get("overall_improvement", 0),
            "implementation_complexity": self._calculate_implementation_complexity(implementation_plan),
            "estimated_completion_time": self._estimate_optimization_completion_time(implementation_plan)
        }
    ...
    def _validate_metric_data(self, metric_data: Dict[str, Any]) -> None:
        ...
        required_fields = ["metric_name", "metric_type", "metric_value", "source_entity"]
    ...
    def _calculate_baseline_statistics(self, metrics: List[EntityDict]) -> Dict[str, Any]:
    ...
    def _generate_performance_alerts(self, anomalies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_performance_statistics(self, metrics: List[EntityDict]) -> Dict[str, Any]:
    ...
    def _calculate_performance_change(
        ...
        change = {
            ...
            "trend_direction": "improving" if current_mean > baseline_mean else "declining" if current_mean < baseline_mean else "stable",
            "change_significance": "significant" if abs(current_mean - baseline_mean) > baseline.get("std_dev", 0) else "minor"
        }
    ...
    def _calculate_trend_statistics(
        ...
        slope_numerator = sum((x_values[i] - x_mean) * (values[i] - y_mean) for i in range(n))
    ...
    def _generate_trend_predictions(self, values: List[float], metric_type: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence": "high" if abs(recent_trend["correlation"]) > 0.8 else "medium" if abs(recent_trend["correlation"]) > 0.5 else "low",
            "prediction_basis": "linear_trend"
        }

    def _calculate_entity_performance_score(self, entity_metrics: List[Dict[str, Any]]) -> float:
        ...
        stability = 1 - (statistics.stdev(recent_values) / avg_value if avg_value != 0 else 1)
    ...
    def _generate_entity_recommendations(
        ...
        if trend_stats["direction"] == "decreasing" and trend_stats["strength"] in ["moderate", "strong"]:
            recommendations.append(f"Performance decline detected for {entity}. Consider investigating root causes.")
        
        if trend_stats["volatility"] > 0.3:
            recommendations.append(f"High volatility detected for {entity}. Consider implementing performance stabilization measures.")
        
        if trend_stats["direction"] == "increasing":
            recommendations.append(f"Positive performance trend for {entity}. Monitor to maintain improvements.")
    ...
    def _calculate_system_health_score(self, metrics: List[EntityDict]) -> float:
        ...
        performance_stability = 1 - (statistics.stdev(values) / avg_performance if avg_performance != 0 else 1)
    ...
    def _generate_system_recommendations(
        ...
        declining_types = [t for t, data in trends_by_type.items() if data.get("trend_direction") == "decreasing"]
        if declining_types:
            recommendations.append(f"Performance decline detected in metric types: {', '.join(declining_types)}")
        
        # Check for unstable entities
        unstable_entities = [e for e, data in trends_by_entity.items() if data.get("trend_strength") == "weak"]
        if unstable_entities:
            recommendations.append(f"Unstable performance in entities: {', '.join(unstable_entities[:3])}")
        
        # System health recommendations
        system_health = overall_trends.get("system_health_score", 0)
        if system_health < 70:
            recommendations.append("Overall system health is below optimal. Consider comprehensive performance review.")
