...
class ServiceCallsEntity(BaseEntity):
    ...
    def create(self, service_call_data: Dict[str, Any]) -> CreateResponse:
        ...
        """
        required_fields = ['accountID', 'ticketID', 'resourceID', 'startDateTime', 'description']
    ...
    def update(self, service_call_id: int, update_data: Dict[str, Any]) -> UpdateResponse:
        """
    ...
    def get_by_technician(
        ...
        if date_range:
            ...
            filters.extend([
                QueryFilter(field='startDateTime', op='gte', value=start_date.isoformat() if hasattr(start_date, 'isoformat') else start_date),
                QueryFilter(field='startDateTime', op='lte', value=end_date.isoformat() if hasattr(end_date, 'isoformat') else end_date)
            ])
        ...
        if status_filter:
            ...
            if status_filter.lower() in status_map:
                filters.append(QueryFilter(field='status', op='eq', value=status_map[status_filter.lower()]))
    ...
    def get_todays_schedule(self, resource_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_technician_schedule(
        ...
        schedule = {
            'resource_id': resource_id,
            'date_range': {
                'start': date_range[0].isoformat() if hasattr(date_range[0], 'isoformat') else date_range[0],
                'end': date_range[1].isoformat() if hasattr(date_range[1], 'isoformat') else date_range[1]
            },
                ...
                if start_time:
                    try:
                        ...
                        if date_key in schedule['daily_breakdown']:
                            schedule['daily_breakdown'][date_key]['travel_time'] += travel_hours
                    except ValueError:
                        pass
        
        return schedule
    
    def get_service_call_metrics(self, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        for call in service_calls:
            ...
            if start_time and actual_start:
                try:
                    scheduled = datetime.fromisoformat(start_time.replace('Z', '+00:00'))
                    actual = datetime.fromisoformat(actual_start.replace('Z', '+00:00'))
                    if abs((actual - scheduled).total_seconds()) <= 900:  # Within 15 minutes
                        on_time_count += 1
                except ValueError:
                    pass
        ...
        if metrics['total_calls'] > 0:
            metrics['completion_rate'] = (metrics['by_status']['completed'] / metrics['total_calls']) * 100
            
            if on_time_count > 0:
                metrics['on_time_percentage'] = (on_time_count / metrics['total_calls']) * 100
        ...
        if metrics['cost_analysis']['total_estimated'] > 0:
            variance = ((metrics['cost_analysis']['total_actual'] - metrics['cost_analysis']['total_estimated']) / 
                       metrics['cost_analysis']['total_estimated']) * 100
            metrics['cost_analysis']['variance_percentage'] = variance
        
        return metrics
    
    def validate_service_call_data(self, service_call_data: Dict[str, Any]) -> Dict[str, Any]:
        """
