...
class ResourcesEntity(BaseEntity):
    ...
    def get_resource_availability(
        ...
        # Get scheduled tasks
        scheduled_tasks = self.client.query("Tasks", filters=[
                        ...
                        while current <= task_end_dt:
                            if current.weekday() < 5:  # Work day
                                ...
                                if date_str in date_to_index:
                                    ...
                                    daily_breakdown[idx]["scheduled_hours"] += hours_per_day
        ...
        return {
            ...
            "status": "overbooked" if utilization_percentage > 100 else
                     "fully_booked" if utilization_percentage >= 90 else
                     "well_utilized" if utilization_percentage >= 70 else
                     "under_utilized",
        }
    ...
    def calculate_utilization(
        ...
        if period == "current_month":
            ...
            start_date = custom_start if isinstance(custom_start, datetime) else datetime.fromisoformat(custom_start.replace("Z", "+00:00"))
            end_date = custom_end if isinstance(custom_end, datetime) else datetime.fromisoformat(custom_end.replace("Z", "+00:00"))
        else:
        ...
        for entry in time_entries:
            ...
            if project_id:
                if project_id not in project_breakdown:
                    project_breakdown[project_id] = {"billable": 0, "non_billable": 0, "total": 0}
            ...
            if task_id:
                if task_id not in task_breakdown:
                    task_breakdown[task_id] = {"billable": 0, "non_billable": 0, "total": 0}
        ...
        billable_utilization = (billable_hours / capacity_hours * 100) if capacity_hours > 0 else 0
        total_utilization = (total_hours / capacity_hours * 100) if capacity_hours > 0 else 0
        ...
        return {
            ...
            "status": {
                "utilization_status": "overutilized" if total_utilization > 100 else
                                    "well_utilized" if total_utilization >= 70 else
                                    "under_utilized",
                "billable_status": "excellent" if billable_utilization >= 80 else
                                 "good" if billable_utilization >= 60 else
                                 "needs_improvement",
                "efficiency_status": "excellent" if efficiency_ratio >= 80 else
                                   "good" if efficiency_ratio >= 60 else
                                   "needs_improvement",
            }
        }
    ...
    def get_workload_summary(
        ...
        # Get current tasks
        active_tasks = self.client.query("Tasks", filters=[
            QueryFilter(field="assignedResourceID", op="eq", value=resource_id),
            QueryFilter(field="status", op="in", value=[1, 2, 3])  # Open statuses
        ])
        ...
        return {
            ...
            "workload_status": "critical" if total_overdue > 0 or availability["utilization_percentage"] > 110 else
                             "high" if availability["utilization_percentage"] > 90 or total_upcoming_deadlines > 3 else
                             "normal" if availability["utilization_percentage"] > 60 else
                             "light",
        }
    ...
    def add_skill(
        ...
        if skill_level not in [1, 2, 3, 4]:
            raise ValueError("skillLevel must be between 1 and 4 (use SKILL_LEVEL constants)")
    ...
    def get_resources_by_skill(
        ...
        if minimum_level:
            ...
            filters.append(QueryFilter(field="skillLevel", op="gte", value=minimum_level))
            
        # Get resource skills
        resource_skills = self.client.query("ResourceSkills", filters=filters, max_records=limit)
        ...
        if resource_ids:
            ...
            if active_only:
                resource_filters.append(QueryFilter(field="Active", op="eq", value=True))
            ...
            for skill_record in resource_skills.get("items", []):
                ...
                if resource_data:  # Only include if resource found (handles active_only filter)
                    resources_with_skills.append({
                        ...
                        "skill_details": {
                            ...
                            "certification_date": skill_record.get("certificationDate"),
    ...
    def match_resources_to_requirements(
        ...
        for resource in all_resources.get("items", []):
            ...
            resource_skill_map = {
                skill.get("skillID"): skill for skill in resource_skills
            ...
            for req in required_skills:
                ...
                if skill_id in resource_skill_map:
                    resource_skill_level = resource_skill_map[skill_id].get("skillLevel", 0)
            ...
            match_data = {
                ...
                "match_quality": "excellent" if overall_score >= 90 else
        ...
        return {
            ...
            "matching_statistics": {
                ...
                "fair_matches": len([m for m in matches if m["match_quality"] == "fair"]),
                "poor_matches": len([m for m in matches if m["match_quality"] == "poor"]),
            },
        }
    ...
    def update_resource_skill(
        ...
        resource_skills = self.client.query("ResourceSkills", filters=[
    ...
    def get_resource_calendar(
        ...
        while current_date <= end_dt:
            ...
            for item in scheduled_items:
                ...
                if item["type"] == "time_entry" and item["date"] == date_str:
                    ...
                    task_start = datetime.fromisoformat(item["start_date"].replace("Z", "+00:00"))
                    task_end = datetime.fromisoformat(item["end_date"].replace("Z", "+00:00"))
                    if task_start.date() <= current_date.date() <= task_end.date():
                        ...
                        work_days_in_task = sum(1 for i in range(task_days) 
                                              if (task_start.date() + timedelta(days=i)).weekday() < 5)
                        if work_days_in_task > 0 and is_work_day:
                            daily_task_hours = item["estimated_hours"] / work_days_in_task
            ...
            calendar_data["daily_schedule"].append({
                ...
                "items": daily_items,
                "utilization_status": "overbooked" if daily_hours > 8 else
                                   "full" if daily_hours >= 8 else
                                   "partial" if daily_hours > 0 else
                                   "free",
            })
        ...
        calendar_data["summary"]["lightest_day"] = lightest_day if min_daily_hours != float('inf') else None
        ...
        for day in availability["daily_breakdown"]:
            if day["is_work_day"] and day["scheduled_hours"] > max_hours_per_day:
                conflicts.append({
                    ...
                    "excess_hours": round(day["scheduled_hours"] - max_hours_per_day, 2),
                    "severity": "high" if day["scheduled_hours"] > max_hours_per_day * 1.25 else "medium"
                })
    ...
    def reschedule_resource(
        ...
        if task.get("assignedResourceID") != resource_id:
            raise ValueError(f"Task {task_id} is not assigned to resource {resource_id}")
    ...
    def calculate_resource_cost(
        ...
        if work_type_id:
            rate_filters.append(QueryFilter(field="workTypeID", op="eq", value=work_type_id))
        ...
        for rate in billing_rates.get("items", []):
            ...
            if rate_effective:
                try:
                    eff_dt = datetime.fromisoformat(rate_effective.replace("Z", "+00:00"))
        ...
        cost_breakdown = {
            ...
            "costs": {
                ...
                "margin_percentage": round(
                    ((billable_cost - internal_cost) / billable_cost * 100) if billable_cost > 0 else 0, 2
                ),
            },
            "overhead": {},
        }
        ...
        if include_overhead and overhead_percentage > 0:
            ...
            cost_breakdown["overhead"] = {
                ...
                "margin_percentage_with_overhead": round(
                    ((billable_cost - total_internal_cost) / billable_cost * 100) if billable_cost > 0 else 0, 2
                ),
            }
    ...
    def get_rate_history(
        ...
        if start_date:
            if isinstance(start_date, str):
                ...
                start_dt = start_date
            filters.append(QueryFilter(field="effectiveDate", op="gte", value=start_dt.isoformat()))
        ...
        if end_date:
            if isinstance(end_date, str):
                ...
                end_dt = end_date
            filters.append(QueryFilter(field="effectiveDate", op="lte", value=end_dt.isoformat()))
    ...
    def get_resources_by_role(
        ...
        resource_roles = self.client.query("ResourceRoles", filters=filters, max_records=limit)
    ...
    def remove_role_assignment(
        ...
        if not role_assignments.get("items"):
            raise ValueError(f"Active role assignment for role {role_id} not found for resource {resource_id}")
    ...
    def validate_skill_assignment(
            ...
            for existing_skill in existing_skills:
                if existing_skill.get("skillID") == skill_id:
                    ...
                    if current_level == skill_level:
                        warnings.append("Resource already has this skill at the same level")
        ...
        return {
            ...
            "skill_assignment": {
                ...
                "resource_name": f"{resource.get('FirstName', '')} {resource.get('LastName', '')}".strip() if resource else "Unknown",
            }
        }
    ...
    def validate_billing_rate(
        ...
        if internal_rate is not None:
            if internal_rate < 0:
                ...
                warnings.append("Internal rate is higher than billing rate (negative margin)")
            elif internal_rate == hourly_rate:
        ...
        if resource and internal_rate is not None:
            margin_percentage = ((hourly_rate - internal_rate) / hourly_rate * 100) if hourly_rate > 0 else 0
        ...
        return {
            ...
            "rate_analysis": {
                "hourly_rate": hourly_rate,
                "internal_rate": internal_rate,
                "margin_percentage": round(margin_percentage, 2) if internal_rate is not None else None,
