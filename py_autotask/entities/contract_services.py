...
class ContractServicesEntity(BaseEntity):
    ...
    def update(self, contract_service_id: int, update_data: Dict[str, Any]) -> UpdateResponse:
        """
    ...
    def get_active_services(self, contract_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def calculate_service_total(self, contract_service_data: Dict[str, Any], quantity: float = 1.0) -> Dict[str, float]:
        """
        ...
        if 'unitCost' in contract_service_data and contract_service_data['unitCost'] is not None:
    ...
    def bulk_update_pricing(self, contract_id: int, price_adjustments: Dict[int, Dict[str, Any]]) -> List[UpdateResponse]:
        """
    ...
    def validate_service_pricing(self, contract_service_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if 'unitPrice' not in contract_service_data or contract_service_data['unitPrice'] is None:
            errors.append("Unit price is required")
        elif float(contract_service_data['unitPrice']) < 0:
            errors.append("Unit price cannot be negative")
        
        # Check cost vs price relationship
        if ('unitCost' in contract_service_data and 
            'unitPrice' in contract_service_data and 
            contract_service_data['unitCost'] is not None and
            contract_service_data['unitPrice'] is not None):
        ...
        if ('adjustedPrice' in contract_service_data and 
            'unitPrice' in contract_service_data and
            contract_service_data['adjustedPrice'] is not None and
            contract_service_data['unitPrice'] is not None):
            ...
            if adjusted_price != unit_price:
                warnings.append(f"Adjusted price ({adjusted_price}) differs from unit price ({unit_price})")
