...
from ..types import QueryFilter, CreateResponse, UpdateResponse
...
class PurchaseOrdersEntity(BaseEntity):
    ...
    def update(self, purchase_order_id: int, update_data: Dict[str, Any]) -> UpdateResponse:
        """
    ...
    def query(self, filters: Optional[List[QueryFilter]] = None, **kwargs) -> List[Dict[str, Any]]:
        """
    ...
    def get_by_vendor(self, vendor_account_id: int) -> List[Dict[str, Any]]:
        ...
        Raises:
            AutotaskAPIError: If the API request fails
        """
        filters = [QueryFilter(field="vendorAccountID", op="eq", value=vendor_account_id)]
        return self.query(filters)
    
    def get_by_status(self, status: str, vendor_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        if vendor_id:
            filters.append(QueryFilter(field="vendorAccountID", op="eq", value=vendor_id))
    ...
    def get_by_date_range(self, start_date: date, end_date: date,
        ...
        """
        filters = [
            QueryFilter(field="purchaseOrderDate", op="gte", value=start_date.isoformat()),
            QueryFilter(field="purchaseOrderDate", op="lte", value=end_date.isoformat())
        ]
    ...
    def get_pending_orders(self, vendor_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_overdue_orders(self, as_of_date: Optional[date] = None) -> List[Dict[str, Any]]:
        """
        ...
        filters = [
            QueryFilter(field="expectedDeliveryDate", op="lt", value=as_of_date.isoformat()),
    ...
    def calculate_order_totals(self, purchase_order_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def approve_order(self, purchase_order_id: int, approved_by: int,
                     approval_notes: Optional[str] = None) -> UpdateResponse:
        """
    ...
    def reject_order(self, purchase_order_id: int, rejected_by: int,
                    rejection_reason: Optional[str] = None) -> UpdateResponse:
        """
    ...
    def mark_as_delivered(self, purchase_order_id: int, delivery_date: Optional[date] = None,
                         received_by: Optional[int] = None) -> UpdateResponse:
        """
    ...
    def cancel_order(self, purchase_order_id: int, cancelled_by: int,
                    cancellation_reason: Optional[str] = None) -> UpdateResponse:
        """
    ...
    def get_vendor_spending_summary(self, vendor_id: int, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        if summary['total_orders'] > 0:
            summary['average_order_value'] = summary['total_amount'] / summary['total_orders']
        else:
            summary['average_order_value'] = 0.0
        
        return summary
    
    def get_procurement_summary(self, date_range: Optional[tuple] = None) -> Dict[str, Any]:
        """
        ...
        summary['top_vendors'] = [
            ...
            for vendor_id, data in top_vendors
        ]
        
        return summary
    
    def validate_purchase_order_data(self, purchase_order_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if expected_delivery_date:
            try:
                if isinstance(expected_delivery_date, str):
                    delivery_date_obj = datetime.fromisoformat(expected_delivery_date.replace('Z', '+00:00')).date()
                    if po_date:
                        po_date_obj = datetime.fromisoformat(po_date.replace('Z', '+00:00')).date()
                        if delivery_date_obj < po_date_obj:
                            errors.append("Expected delivery date cannot be before purchase order date")
