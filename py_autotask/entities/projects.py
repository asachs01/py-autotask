...
class ProjectsEntity(BaseEntity):
    ...
    def set_project_budget(
        self, 
        project_id: int, 
        budget_data: Dict[str, Union[float, int]]
    ) -> ProjectData:
        """
        ...
        for key, value in budget_data.items():
            if isinstance(value, (int, float)) and value < 0:
                raise AutotaskValidationError(f"Budget value for {key} cannot be negative")
    ...
    def track_project_costs(
        ...
        for cost_type, (budget_field, friendly_name) in budget_fields.items():
            if cost_type in cost_data:
                ...
                variance_percent = (variance / budget_value * 100) if budget_value else 0
                ...
                if budget_value > 0:
                    performance = "over_budget" if variance > 0 else "under_budget" if variance < 0 else "on_budget"
    ...
    def get_budget_status(self, project_id: int) -> Dict[str, Any]:
        ...
        actual_labor_cost = sum(float(entry.get("BillableAmount", 0)) for entry in time_entries)
    ...
    def calculate_project_profitability(self, project_id: int) -> Dict[str, Any]:
        ...
        if total_budget > 0:
            ...
            profitability["financial_summary"]["gross_margin_percent"] = round(gross_margin_percent, 2)
            ...
            profitability["profitability_metrics"]["profit_per_hour"] = round(profit_per_hour, 2)
            
            # Cost efficiency (budget vs actual)
            budget_total = budget_status["budgets"]["total_budget"]
            cost_efficiency = (budget_total - total_costs) / budget_total * 100 if budget_total > 0 else 0
            profitability["profitability_metrics"]["cost_efficiency"] = round(cost_efficiency, 2)
    ...
    def allocate_resource(
        ...
        for field in required_fields:
            if field not in allocation_data:
                raise AutotaskValidationError(f"Missing required allocation field: {field}")
        
        # Validate allocation percentage
        allocation_percent = allocation_data["allocation_percentage"]
        if not 0 <= allocation_percent <= 100:
            raise AutotaskValidationError("Allocation percentage must be between 0 and 100")
    ...
    def get_resource_allocation(self, project_id: int) -> Dict[str, Any]:
        ...
        for task in tasks:
            ...
            if resource_id:
                ...
                resource_summary[resource_id]["completion_percentage"] += percent_complete
        ...
        for resource_id, data in resource_summary.items():
            ...
            data["completion_percentage"] = data["completion_percentage"] / task_count if task_count > 0 else 0
            data["utilization"] = (data["total_hours"] / total_estimated_hours * 100) if total_estimated_hours > 0 else 0
    ...
    def optimize_resource_allocation(self, project_id: int) -> Dict[str, Any]:
        ...
        efficiency_score = (balanced_count / total_resources * 100) if total_resources > 0 else 0
        recommendations["efficiency_score"] = round(efficiency_score, 2)
        
        # Add performance insights
        avg_completion = sum(r["completion_percentage"] for r in resources) / len(resources) if resources else 0
    ...
    def update_phase_progress(
        ...
        if not 0 <= progress <= 100:
            raise AutotaskValidationError("Progress percentage must be between 0 and 100")
        ...
        updated_phase = {
            ...
            "status": (
                self.constants.PHASE_STATUS_COMPLETE if progress == 100 
                else self.constants.PHASE_STATUS_IN_PROGRESS if progress > 0 
    ...
    def track_milestone_completion(self, milestone_id: int) -> Dict[str, Any]:
        ...
        milestone_task = self.client.query("Tasks", 
                                         filters=[QueryFilter(field="id", op="eq", value=milestone_id)])
            ...
            try:
                ...
                if due < now:
                    ...
                    if days_remaining > 0 and progress > 0:
                        completion_rate = progress / max(1, (now - due + timedelta(days=30)).days)
                        probability = min(100, completion_rate * days_remaining)
                        milestone_info["impact_analysis"]["completion_probability"] = round(probability, 2)
    ...
    def calculate_gross_margin(self, project_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "gross_margin_percent": profitability["financial_summary"]["gross_margin_percent"],
    ...
    def get_profitability_report(self, project_id: int) -> Dict[str, Any]:
        ...
        if budgeted_cost > 0:
            ...
            cost_performance_index = budgeted_cost / actual_cost if actual_cost > 0 else 0
            
            report["performance_indicators"]["budget_variance"] = budget_variance
            report["performance_indicators"]["cost_performance_index"] = round(cost_performance_index, 3)
    ...
    def compare_actual_vs_estimated(self, project_id: int) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "cost_comparison": {
                ...
                "actual_total": (budget_status["actuals"]["actual_labor_cost"] + 
        ...
        if estimated_cost > 0:
            ...
            comparison["cost_comparison"]["variance_percent"] = round(cost_variance_percent, 2)
            
            # Accuracy score (100% = perfect estimate)
            accuracy = max(0, 100 - abs(cost_variance_percent))
            comparison["accuracy_metrics"]["cost_estimation_accuracy"] = round(accuracy, 2)
        ...
        if estimated_hours > 0:
            ...
            comparison["time_comparison"]["variance_percent"] = round(time_variance_percent, 2)
            
            # Time accuracy score
            time_accuracy = max(0, 100 - abs(time_variance_percent))
            comparison["accuracy_metrics"]["time_estimation_accuracy"] = round(time_accuracy, 2)
        ...
        overall_accuracy = (cost_acc + time_acc) / 2 if (cost_acc > 0 or time_acc > 0) else 0
        comparison["accuracy_metrics"]["overall_accuracy_score"] = round(overall_accuracy, 2)
    ...
    def save_as_template(
        ...
        template = {
            ...
            "name": template_name,
            "description": template_description or f"Template created from project {project.get('ProjectName')}",
    ...
    def get_project_templates(self) -> List[Dict[str, Any]]:
        ...
        mock_templates = [
            ...
            },
            {
                ...
                "phases": ["Assessment", "Planning", "Procurement", "Implementation", "Testing"],
            ...
            },
            {
                ...
                "phases": ["Requirements", "Configuration", "Data Migration", "Training", "Go-Live"],
    ...
    def get_gantt_data(self, project_id: int) -> Dict[str, Any]:
        ...
        if not project_start or not project_end:
            task_dates = [
                task.get("start_date") for task in gantt_tasks 
                if task.get("start_date") and not task["is_milestone"]
            ]
            end_dates = [
                task.get("end_date") for task in gantt_tasks 
        ...
        gantt_data = {
            ...
            "project_duration_days": self._calculate_task_duration(project_start, project_end),
            "tasks": gantt_tasks,
            ...
            "timeline_metadata": {
                ...
                "completed_tasks": len([t for t in gantt_tasks if t["progress"] >= 1.0]),
                "overdue_tasks": len([t for t in gantt_tasks if self._is_task_overdue(t)]),
    ...
    def add_task_dependency(
        ...
        task = self.client.query("Tasks", filters=[QueryFilter(field="id", op="eq", value=task_id)])
        predecessor = self.client.query("Tasks", filters=[QueryFilter(field="id", op="eq", value=depends_on_id)])
        ...
        predecessor_list = [p.strip() for p in current_predecessors.split(",") if p.strip()]
    ...
    def calculate_critical_path(self, project_id: int) -> List[Dict[str, Any]]:
        ...
        for task in tasks:
            ...
            duration = float(task.get("estimatedHours", 0)) / 8  # Convert to days (8 hours/day)
        ...
        def forward_pass():
            ...
            def process_task(task_id):
                ...
                if not task_data["predecessors"]:
                    ...
                    for pred_id in task_data["predecessors"]:
                        if pred_id in task_network:
                            predecessor_finishes.append(task_network[pred_id]["early_finish"])
                    
                    task_data["early_start"] = max(predecessor_finishes) if predecessor_finishes else 0
                
                task_data["early_finish"] = task_data["early_start"] + task_data["duration"]
        ...
        def backward_pass():
            ...
            project_end_time = max(task_data["early_finish"] for task_data in task_network.values())
            ...
            def process_task_backward(task_id):
                ...
                if not task_data["successors"]:
                    ...
                    task_data["late_finish"] = min(successor_starts) if successor_starts else project_end_time
                
                task_data["late_start"] = task_data["late_finish"] - task_data["duration"]
                processed.add(task_id)
        ...
        critical_tasks = []
        for task_id, task_data in task_network.items():
            task_data["total_float"] = task_data["late_start"] - task_data["early_start"]
            task_data["is_critical"] = abs(task_data["total_float"]) < 0.1  # Consider floating point precision
    ...
    def _get_resource_assignments(self, gantt_tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for task in gantt_tasks:
            ...
            if resource_id and not task["is_milestone"]:
                ...
                assignments[resource_id]["total_hours"] += task.get("estimated_hours", 0)
    ...
    def _has_circular_dependency(self, task_id: int, depends_on_id: int) -> bool:
        ...
        depends_on_task = self.client.query("Tasks", filters=[QueryFilter(field="id", op="eq", value=depends_on_id)])
        
        if depends_on_task:
            predecessors = depends_on_task[0].get("predecessors", "")
            predecessor_ids = [int(p.strip()) for p in predecessors.split(",") if p.strip().isdigit()]
    ...
    def validate_project_data(self, project_data: Dict[str, Any]) -> List[str]:
        ...
        budget_fields = ["ProjectCost", "LaborCost", "ExpenseCost", "MaterialCost", "EstimatedTime"]
    ...
    def get_project_health_score(self, project_id: int) -> Dict[str, Any]:
            ...
            try:
                ...
                if now < start:
                    ...
                    if total_days > 0:
                        ...
                        factors["schedule_health"] = max(20, 100 - abs(timeline_progress - 0.5) * 100)
            except (ValueError, TypeError):
        ...
        if allocation_data["resources_allocated"] > 0:
            avg_utilization = sum(
                r["utilization"] for r in allocation_data["resource_breakdown"]
        ...
        tasks = self.get_project_tasks(project_id)
        if tasks:
            avg_progress = sum(float(task.get("percentComplete", 0)) for task in tasks) / len(tasks)
            factors["progress_health"] = min(100, avg_progress + 20)  # Bonus for any progress
        else:
    ...
    def _generate_health_recommendations(self, factors: Dict[str, float]) -> List[Dict[str, str]]:
