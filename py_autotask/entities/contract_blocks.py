...
class ContractBlocksEntity(BaseEntity):
    ...
    def update(self, contract_block_id: int, update_data: Dict[str, Any]) -> UpdateResponse:
        """
    ...
    def get_active_blocks(self, contract_id: Optional[int] = None, 
    ...
    def get_blocks_by_type(self, contract_id: int, block_type: int) -> List[Dict[str, Any]]:
        """
    ...
    def calculate_block_utilization(self, contract_block_data: Dict[str, Any], 
                                   used_hours: Optional[float] = None,
                                   used_amount: Optional[float] = None) -> Dict[str, Any]:
        """
        ...
        if block_type in [1, 3]:
            ...
            hours_utilization = (used_hours / total_hours * 100) if total_hours > 0 else 0
        ...
        if block_type in [2, 3]:
            ...
            value_utilization = (used_value / total_value * 100) if total_value > 0 else 0
    ...
    def _validate_date_range(self, block_data: Dict[str, Any]) -> None:
        ...
        if isinstance(date_begin, str):
            date_begin = datetime.fromisoformat(date_begin.replace('Z', '+00:00')).date()
