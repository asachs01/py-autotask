...
class UserDefinedFieldsEntity(BaseEntity):
    ...
    def clone_user_defined_field(
        ...
        clone_data = {k: v for k, v in source_field.items() 
                     if k not in ["id", "createdDate", "createdByResourceID", 
                                "lastModifiedDate", "activatedDate", "deactivatedDate"]}
    ...
    def validate_field_definition(
        ...
        if field_name:
            if not re.match(r'^[a-zA-Z][a-zA-Z0-9_]*$', field_name):
                errors.append("Field name must start with letter and contain only letters, numbers, and underscores")
            if len(field_name) > 50:
                errors.append("Field name cannot exceed 50 characters")
        
        # Validate data type
        valid_data_types = ["Text", "Integer", "Decimal", "Date", "DateTime", "Boolean", "Picklist", "MultiPicklist"]
        ...
        if data_type and field_type:
            if data_type in valid_field_types and field_type not in valid_field_types[data_type]:
                errors.append(f"Field type '{field_type}' is not compatible with data type '{data_type}'")
            ...
            if not picklist_values:
                ...
                for i, value in enumerate(picklist_values):
                    if not isinstance(value, dict):
                        ...
                        errors.append(f"Picklist value {i+1} must have 'value' and 'label' fields")
        ...
        for i, rule in enumerate(validation_rules):
            ...
            if not rule_validation["is_valid"]:
                errors.extend([f"Validation rule {i+1}: {error}" for error in rule_validation["errors"]])
        
        # Validate default value compatibility
        default_value = field_definition.get("defaultValue")
        if default_value is not None:
            value_validation = self._validate_field_value(default_value, data_type, field_definition)
            if not value_validation["is_valid"]:
                errors.append(f"Default value is not compatible with field type: {value_validation['error']}")
    ...
    def export_udf_schemas(
        ...
        if entity_types:
            if len(entity_types) == 1:
                filters.append({"field": "entityType", "op": "eq", "value": entity_types[0]})
            else:
        ...
        if export_format.lower() == "json":
            export_data["formatted_data"] = json.dumps(export_data, indent=2, default=str)
        ...
        for entity_type, fields in schemas.items():
            for field_data in fields:
                ...
                try:
                    ...
                    existing_fields = self.get_user_defined_fields_by_entity(entity_type, active_only=False)
                    existing_field = next((f for f in existing_fields if f.get("name") == field_name), None)
                    
                    if existing_field:
                        if import_options.get("update_existing", False):
                            # Update existing field
                            update_data = {k: v for k, v in field_data.items() 
                                         if k not in ["id", "createdDate", "createdByResourceID"]}
                            result = self.update_by_id(existing_field["id"], update_data)
    ...
    def get_user_defined_fields_summary(
        ...
        if include_usage_stats:
            ...
            usage_stats = {
                "most_used_data_type": max(by_data_type.items(), key=lambda x: x[1])[0] if by_data_type else None,
                "most_used_field_type": max(by_field_type.items(), key=lambda x: x[1])[0] if by_field_type else None,
                "entity_with_most_fields": max(by_entity_type.items(), key=lambda x: x[1]["total"])[0] if by_entity_type else None,
                "average_fields_per_entity": round(total_fields / len(by_entity_type), 2) if by_entity_type else 0,
                "field_utilization_rate": round((active_fields / total_fields * 100), 2) if total_fields > 0 else 0
            }
    ...
    def bulk_deactivate_user_defined_fields(
        ...
        for field_id in field_ids:
            try:
                result = self.deactivate_user_defined_field(field_id, deactivation_reason, preserve_data)
                results.append({"id": field_id, "success": True, "result": result})
            except Exception as e:
                results.append({"id": field_id, "success": False, "error": str(e)})
    ...
    def analyze_field_usage_patterns(
        ...
        fields = self.get_user_defined_fields_by_entity(entity_type) if entity_type else self.query_all()
        
        # Sample usage data analysis
        usage_analysis = {
            "analysis_period": {"from": date_from.isoformat(), "to": date_to.isoformat()},
        ...
        for field in fields:
            ...
            field_stats = {
                ...
                "most_common_values": ["Value1", "Value2", "Value3"],  # Sample common values
    ...
    def monitor_field_performance(self) -> Dict[str, Any]:
        ...
        if performance_data["performance_metrics"]["field_lookup_performance"]["p99_response_time_ms"] > 100:
            performance_data["alerts"].append({
        ...
        if performance_data["performance_metrics"]["validation_performance"]["validation_success_rate"] < 95:
            performance_data["alerts"].append({
    ...
    def _validate_validation_rule(
        ...
        compatible_rules = {
            ...
            "Decimal": ["required", "min_value", "max_value", "range", "decimal_places"],
        ...
        if data_type in compatible_rules and rule_type not in compatible_rules[data_type]:
            errors.append(f"Validation rule type '{rule_type}' is not compatible with data type '{data_type}'")
        ...
        try:
            if data_type == "Text":
                ...
                if max_length and len(value) > max_length:
                    return {"is_valid": False, "error": f"Value exceeds maximum length ({max_length})"}
                ...
                if isinstance(value, str):
                    try:
                        ...
                        return {"is_valid": False, "error": "Value must be a valid ISO date/datetime string"}
