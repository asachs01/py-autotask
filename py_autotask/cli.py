    ...
    try:
        ...
        if output == 'json':
            ...
            if 'entityInformation' in entity_info:
                ...
                click.echo(f"Has User Defined Fields: {info.get('hasUserDefinedFields', 'N/A')}")
...
@click.pass_context
def create(ctx: click.Context, entity: str, data_file, batch_size: int, output: str) -> None:
    ...
    try:
        ...
        click.echo(f"Creating {len(entities_data)} {entity} entities in batches of {batch_size}...")
        ...
        if output == 'json':
            ...
            click.echo(f"✓ Successfully created {success_count}/{len(entities_data)} entities")
...
@click.option('--output', '-o', type=click.Choice(['json', 'summary']), default='summary')
@click.pass_context
def update(ctx: click.Context, entity: str, data_file, batch_size: int, output: str) -> None:
    ...
    try:
        ...
        click.echo(f"Updating {len(entities_data)} {entity} entities in batches of {batch_size}...")
...
@click.option('--batch-size', type=int, default=200, help='Batch size (max 200)')
@click.option('--ids-file', type=click.File('r'), help='File with entity IDs (one per line)')
@click.option('--confirm', is_flag=True, help='Skip confirmation prompt')
@click.pass_context
def delete(ctx: click.Context, entity: str, ids: tuple, batch_size: int, ids_file, confirm: bool) -> None:
    ...
    try:
        ...
        click.echo(f"Deleting {len(entity_ids)} {entity} entities in batches of {batch_size}...")
...
def delete_attachment(ctx: click.Context, attachment_id: int, confirm: bool) -> None:
    ...
    try:
        ...
        # Confirmation prompt
        if not confirm:
            if not click.confirm(f"Delete attachment '{attachment.title}' ({attachment.file_name})?"):
                click.echo("Aborted.")
                return
